...
#define FPGA_POWER_CYCLE_CFG (0x3b8)
                                        ...
                                        char *buf)
{
    ...
    if (NULL != clounix_fpga_base)
    {
        data = readl(clounix_fpga_base + FPGA_POWER_CYCLE_CFG);
    }
                                        ...
                                        const char *buf, size_t count)
{
    ...
    if (NULL != clounix_fpga_base)
    {
        writel(value, clounix_fpga_base + FPGA_POWER_CYCLE_CFG);
    }
...
static int drv_xilinx_fpga_probe(struct pci_dev *pdev, const struct pci_device_id *pci_id)
{
    int err;
    void __iomem *base;
    ...
    if (pci_find_capability(pdev, PCI_CAP_ID_MSI) == 0) {
        LOG_ERR(CLX_DRIVER_TYPES_FPGA, "%s[%d] MSI not supported.\n", __func__, __LINE__);
        return -EPERM;
    }

    err = pci_enable_device(pdev);
    if (err) {
        LOG_ERR(CLX_DRIVER_TYPES_FPGA, "%s[%d] failed to enable device.\n", __func__, __LINE__);
        return err;
    }
    ...
    if (!devm_request_mem_region(&pdev->dev, start, len, "clounix_fpga")) {
        LOG_ERR(CLX_DRIVER_TYPES_FPGA, "%s[%d] failed to request iomem 0x%llx.\n", __func__, __LINE__, start);
        err = -EBUSY;
        goto err_disable;
    }
    ...
    base = devm_ioremap(&pdev->dev, start, len);
    if (!base) {
        LOG_ERR(CLX_DRIVER_TYPES_FPGA, "%s[%d] ioremap failed.\n", __func__, __LINE__);
        err = -EIO;
        goto err_disable;
    }

    WRITE_ONCE(clounix_fpga_base, base);
    pci_set_drvdata(pdev, base);

    LOG_INFO(CLX_DRIVER_TYPES_FPGA, "supports %d MSI vector(s)\n", pci_msi_vec_count(pdev));

    err = pci_alloc_irq_vectors(pdev, 1, 1, PCI_IRQ_MSI);
    if (err < 0) {
        LOG_ERR(CLX_DRIVER_TYPES_FPGA, "%s[%d] failed to allocate MSI vectors.\n", __func__, __LINE__);
        goto err_disable;
    }

    err = request_irq(pci_irq_vector(pdev, 0), clounix_fpga_irq_hd, IRQF_SHARED,
                      pdev->driver->name, pdev);
    if (err < 0) {
        LOG_ERR(CLX_DRIVER_TYPES_FPGA, "%s[%d] failed to request IRQ.\n", __func__, __LINE__);
        goto err_free_vectors;
    }

    writel(IRQ_CTRL_CFG_RST_BIT, base + FPGA_IRQ_CTRL_ADDR);

    reboot_nb.notifier_call = sys_led_reboot_work;
    err = register_reboot_notifier(&reboot_nb);
    if (err) {
        LOG_ERR(CLX_DRIVER_TYPES_FPGA, "failed to register reboot notifier: %d\n", err);
        goto err_free_irq;
    }

    err = sysfs_create_file(&pdev->dev.kobj, &attr.attr);
    if (err) {
        LOG_ERR(CLX_DRIVER_TYPES_FPGA, "sysfs_create_file failed: %d\n", err);
    }

    LOG_INFO(CLX_DRIVER_TYPES_FPGA, "FPGA driver initialized successfully\n");
    return 0;
...
err_disable:
    WRITE_ONCE(clounix_fpga_base, NULL);
    pci_clear_master(pdev);
    pci_disable_device(pdev);
    return err;
}

static void drv_xilinx_fpga_remove(struct pci_dev *pdev)
{
    void __iomem *base = READ_ONCE(clounix_fpga_base);

    if (base)
        writel(IRQ_CTRL_DISABLE_INT, base + FPGA_IRQ_CTRL_ADDR);

    sysfs_remove_file(&pdev->dev.kobj, &attr.attr);

    if (pci_irq_vector(pdev, 0) >= 0)
        free_irq(pci_irq_vector(pdev, 0), pdev);
    pci_free_irq_vectors(pdev);
