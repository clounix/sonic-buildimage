...
static ssize_t clx_get_psu_debug(char *buf, size_t count)
{
    return sprintf(buf, "check PSU info: \n"
                        "cat /sys/s3ip/psu/psu*/*\n"
                        "check PSU temerature: \n"
                        "cat /sys/s3ip/psu/psu1/temp*/*\n");
}
...
static ssize_t clx_get_psu_out_min_vol(unsigned int psu_index, char *buf, size_t count)
{
    struct psu_fn_if *psu_dev = get_psu();

    PSU_DEV_VALID(psu_dev);
    PSU_DEV_VALID(psu_dev->get_psu_out_min_vol);
    return psu_dev->get_psu_out_min_vol(psu_dev, psu_index, buf, count);
}

static ssize_t clx_get_psu_out_max_vol(unsigned int psu_index, char *buf, size_t count)
{
    struct psu_fn_if *psu_dev = get_psu();

    PSU_DEV_VALID(psu_dev);
    PSU_DEV_VALID(psu_dev->get_psu_out_max_vol);
    return psu_dev->get_psu_out_max_vol(psu_dev, psu_index, buf, count);
}
...
static struct s3ip_sysfs_psu_drivers_s drivers = {
    ...
    .get_psu_out_min_vol = clx_get_psu_out_min_vol,
    .get_psu_out_max_vol = clx_get_psu_out_max_vol,
