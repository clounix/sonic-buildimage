...
class PSUTC(TestCaseCommon):
    ...
    def __init__(self, index, logger, platform_cfg_file, case_cfg_file=None):
        ...
        # psu_module = load_platform_util_module(self.__PLATFORM_SPECIFIC_MODULE_NAME)
        # try:
    ...
    def get_psu_count(self):
        try:
            return int(self.read_sysfs("/sys/s3ip/psu/num"),10)
        except Exception as e:
    ...
    '''
        ...
        for n in range(self.psu_count):
            model_name = self.read_sysfs('/sys/s3ip/psu/psu{n}/model_name'.format(n=n+1))
            if not model_name :
                self.logger.log_err("FAIL to read psu {n} model_name".format(n=n+1), True)
                return E.EPSU5001

            part_number = self.read_sysfs('/sys/s3ip/psu/psu{n}/part_number'.format(n=n+1))
            if not part_number :
            ...
            #     return E.EPSU5001
            serial_number = self.read_sysfs('/sys/s3ip/psu/psu{n}/serial_number'.format(n=n+1))
    ...
    def test_psu_status(self):
        ...
        for n in range(self.psu_count):
            present = self.read_sysfs('/sys/s3ip/psu/psu{n}/status'.format(n=n+1))
            """
                present status:
                0: ABSENT, 不在位
                1: OK, 在位且正常（在位上电）
            ...
            in_status = self.read_sysfs('/sys/s3ip/psu/psu{n}/in_status'.format(n=n+1))
            if in_status != "1":
                self.logger.log_err("psu {} input AC error !".format(n+1), True)
                return E.EPSU5000

            in_curr = int(self.read_sysfs('/sys/s3ip/psu/psu{n}/in_curr'.format(n=n+1)))
            ...
            in_vol = int(self.read_sysfs('/sys/s3ip/psu/psu{n}/in_vol'.format(n=n+1)))
            ...
            in_power = int(self.read_sysfs('/sys/s3ip/psu/psu{n}/in_power'.format(n=n+1)))
            ...
            # output
            out_status = self.read_sysfs('/sys/s3ip/psu/psu{n}/out_status'.format(n=n+1))
            if out_status != "1":
                self.logger.log_err("psu {} input AC error !".format(n+1), True)
                return E.EPSU5000

            out_curr = int(self.read_sysfs('/sys/s3ip/psu/psu{n}/out_curr'.format(n=n+1)))
            ...
            out_vol = int(self.read_sysfs('/sys/s3ip/psu/psu{n}/out_vol'.format(n=n+1)))
            ...
            out_power = int(self.read_sysfs('/sys/s3ip/psu/psu{n}/out_power'.format(n=n+1)))
