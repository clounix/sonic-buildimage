...
static ssize_t drv_get_watchdog_identify(void *driver, char *buf, size_t count)
{
   int ret = -1;
    ...
    ret = scnprintf(buf, count,"clounix_fpga_wdt\n");
    return ret;
}
...
static ssize_t drv_get_watchdog_state(void *driver, char *buf, size_t count)
{
    ...
    if (NULL != clounix_fpga_base) {
        data= readl(clounix_fpga_base + WATCHDOG_CONFIG);
        if(data & WATCHDOG_BIT(CONFIG_ENABLE))
        {
            ret = scnprintf(buf, count, "active\n");
        }
        else
        {
            ret = scnprintf(buf, count, "inactive\n");
        };
    }
    else {
...
static ssize_t drv_get_watchdog_timeleft(void *driver, char *buf, size_t count)
{
    ...
    timeleft = readl(clounix_fpga_base + WATCHDOG_STATUS);
...
static ssize_t drv_get_watchdog_timeout(void *driver, char *buf, size_t count)
{
    ...
    timeout = readl(clounix_fpga_base + WATCHDOG_CONFIG);
...
static int drv_set_watchdog_timeout(void *driver, int value)
{
    ...
    timeout = readl(clounix_fpga_base + WATCHDOG_CONFIG);
    writel(WATCHDOG_BFINS(CONFIG_TIMEOUT,value,timeout), clounix_fpga_base + WATCHDOG_CONFIG);
...
static int drv_set_watchdog_enable_status(void *driver, int value)
{
    ...
    data= readl(clounix_fpga_base + WATCHDOG_CONFIG);
    writel(WATCHDOG_BFINS(CONFIG_ENABLE,value,data), clounix_fpga_base + WATCHDOG_CONFIG);
    return 0;
}
...
static int drv_set_watchdog_reset(void *driver, int value)
{
    ...
    writel(data, clounix_fpga_base + WATCHDOG_FEED);
    return 0;
}
...
int drv_watchdog_fpga_init(void **watchdog_driver)
{
