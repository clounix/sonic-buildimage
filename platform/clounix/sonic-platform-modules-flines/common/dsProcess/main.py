            ...
            if int(c) == 1:
                if os.path.exists(signExecutable):
                    ...
                    syslog.syslog(syslog.LOG_INFO,
                                  'vtysh process exit code: {0} '.format(r))
                else:
                    syslog.syslog(
                        syslog.LOG_WARNING, 'vtysh cannot find executable {0} '.format(signExecutable))
            else:
                syslog.syslog(syslog.LOG_INFO,
                              'vtysh skip test {0} '.format(c))
    else:
        syslog.syslog(syslog.LOG_WARNING,
                      'vtysh cannot find mark file {0}'.format(signPath))
