...
static ssize_t clx_get_main_board_fpga_debug(char *buf, size_t count)
{
    return sprintf(buf, "check FPGA version: \n"
                        "cat /sys/s3ip/fpga/fpga*/firmware_version\n");
}
...
static ssize_t clx_get_main_board_fpga_power_history_reg(unsigned int fpga_index, char *buf, size_t count)
{
    int ret;
    struct fpga_fn_if *fpga_dev = get_fpga();

    FPGA_DEV_VALID(fpga_dev);
    FPGA_DEV_VALID(fpga_dev->get_main_board_fpga_power_history_reg);
    ret = fpga_dev->get_main_board_fpga_power_history_reg(fpga_dev, fpga_index, buf, count);
    if (ret < 0)
    {
        return -ENOSYS;
    }
...
static ssize_t clx_get_main_board_fpga_power_history_ctrl(unsigned int fpga_index, char *buf, size_t count)
{
    int ret;
    struct fpga_fn_if *fpga_dev = get_fpga();

    FPGA_DEV_VALID(fpga_dev);
    FPGA_DEV_VALID(fpga_dev->get_main_board_fpga_power_history_ctrl);
    ret = fpga_dev->get_main_board_fpga_power_history_ctrl(fpga_dev, fpga_index, buf, count);
    if (ret < 0)
    {
        return -ENOSYS;
    }
 ...
 * otherwise it returns a negative value on failed.
 */
static int clx_set_main_board_fpga_power_history_ctrl(unsigned int fpga_index, unsigned int value)
{
    int ret;
    struct fpga_fn_if *fpga_dev = get_fpga();

    FPGA_DEV_VALID(fpga_dev);
    FPGA_DEV_VALID(fpga_dev->set_main_board_fpga_power_history_ctrl);
    ret = fpga_dev->set_main_board_fpga_power_history_ctrl(fpga_dev, fpga_index, value);
    if (ret < 0)
    {
        return -ENOSYS;
    }
...
static struct s3ip_sysfs_fpga_drivers_s drivers = {
    ...
    .get_main_board_fpga_power_history_reg = clx_get_main_board_fpga_power_history_reg,
    .get_main_board_fpga_power_history_ctrl = clx_get_main_board_fpga_power_history_ctrl,
    .set_main_board_fpga_power_history_ctrl = clx_set_main_board_fpga_power_history_ctrl,
