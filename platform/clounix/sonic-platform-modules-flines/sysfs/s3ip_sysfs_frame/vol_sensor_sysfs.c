...
static ssize_t vol_sensor_debug_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    int ret;

    check_p(g_vol_sensor_drv);
    check_p(g_vol_sensor_drv->get_debug);

    ret = g_vol_sensor_drv->get_debug(buf, PAGE_SIZE);
    if (ret < 0)
    {
        VOL_SENSOR_ERR("get sensor debug failed, ret: %d\n", ret);
        return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SYSFS_DEV_ERROR);
    }
    return ret;
}

static ssize_t vol_sensor_debug_store(struct switch_obj *obj, struct switch_attribute *attr,
                                  const char *buf, size_t count)
{
    int ret;

    check_p(g_vol_sensor_drv);
    check_p(g_vol_sensor_drv->set_debug);

    ret = g_vol_sensor_drv->set_debug(buf, PAGE_SIZE);
    if (ret < 0)
    {
        VOL_SENSOR_ERR("set sensor debug failed, ret: %d\n", ret);
    }
    return ret;
}

static ssize_t vol_sensor_loglevel_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    int ret;

    check_p(g_vol_sensor_drv);
    check_p(g_vol_sensor_drv->get_loglevel);

    ret = g_vol_sensor_drv->get_loglevel(buf, PAGE_SIZE);
    if (ret < 0)
    {
        VOL_SENSOR_ERR("get sensor loglevel failed, ret: %d\n", ret);
        return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SYSFS_DEV_ERROR);
    }
    return ret;
}

static ssize_t vol_sensor_loglevel_store(struct switch_obj *obj, struct switch_attribute *attr,
                                     const char *buf, size_t count)
{
    int ret;

    check_p(g_vol_sensor_drv);
    check_p(g_vol_sensor_drv->set_loglevel);

    ret = g_vol_sensor_drv->set_loglevel(buf, PAGE_SIZE);
    if (ret < 0)
    {
        VOL_SENSOR_ERR("set sensor loglevel failed, ret: %d\n", ret);
    }
    return ret;
}
...
static ssize_t vol_sensor_crit_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    unsigned int vol_index;
    int ret;

    check_p(g_vol_sensor_drv);
    check_p(g_vol_sensor_drv->get_main_board_vol_crit);

    vol_index = obj->index;
    VOL_SENSOR_DBG("vol index: %u\n", vol_index+1);
    ret = g_vol_sensor_drv->get_main_board_vol_crit(vol_index, buf, PAGE_SIZE);
    if (ret < 0) {
        VOL_SENSOR_ERR("get vol%u crit failed, ret: %d\n", vol_index+1, ret);
        return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SYSFS_DEV_ERROR);
    }
    return ret;
}

static ssize_t vol_sensor_crit_store(struct switch_obj *obj, struct switch_attribute *attr,
                   const char* buf, size_t count)
{
    unsigned int vol_index;
    int ret;
    ...
    check_p(g_vol_sensor_drv);
    check_p(g_vol_sensor_drv->set_main_board_vol_crit);

    vol_index = obj->index;
    VOL_SENSOR_DBG("vol index: %u\n", vol_index+1);
    ret = g_vol_sensor_drv->set_main_board_vol_crit(vol_index, buf, count);
    if (ret < 0) {
        VOL_SENSOR_ERR("set vol%u crit failed, value: %s, count: %lu, ret: %d\n",
            vol_index+1, buf, count, ret);
        return -EIO;
    }
    VOL_SENSOR_DBG("set vol%u crit success, value: %s, count: %lu, ret: %d\n",
        vol_index+1, buf, count, ret);
    return count;
}
...
static struct switch_attribute vol_sensor_debug_attr = __ATTR(debug, S_IRUGO | S_IWUSR, vol_sensor_debug_show, vol_sensor_debug_store);
static struct switch_attribute vol_sensor_loglevel_attr = __ATTR(loglevel, S_IRUGO | S_IWUSR, vol_sensor_loglevel_show, vol_sensor_loglevel_store);

static struct attribute *vol_sensor_dir_attrs[] = {
    &num_vol_att.attr,
    &vol_sensor_debug_attr.attr,
    &vol_sensor_loglevel_attr.attr,
    NULL,
};
...
static struct switch_attribute vol_crit_attr = __ATTR(crit,  S_IRUGO | S_IWUSR, vol_sensor_crit_show, vol_sensor_crit_store);
...
static struct switch_attribute vol_alarm_attr = __ATTR(alarm, S_IRUGO, vol_sensor_alarm_show, NULL);
...
static int vol_sensor_sub_single_create_kobj_and_attrs(struct kobject *parent, unsigned int index)
{
    ...
    vol_sensor = &g_vol_sensor.vol[index];
    memset(name, 0, sizeof(name));
    snprintf(name, sizeof(name), "vol%u", index+1);
...
static int vol_sensor_sub_create_kobj_and_attrs(struct kobject *parent, int vol_num)
{
    ...
    for (vol_index = 0; vol_index < vol_num; vol_index++) {
...
static void vol_sensor_sub_remove(void)
{
    ...
    if (g_vol_sensor.vol) {
        for (vol_index = 0; vol_index < g_vol_sensor.vol_number; vol_index++) {
