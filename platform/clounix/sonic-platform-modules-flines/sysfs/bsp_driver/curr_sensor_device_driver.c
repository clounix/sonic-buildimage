...
/*************************************main board current***************************************/
static ssize_t clx_get_main_board_curr_loglevel(char *buf, size_t count)
{
    PRINT_LOGLEVEL(g_dev_loglevel[CLX_DRIVER_TYPES_CURR], buf, count);
}

static ssize_t clx_set_main_board_curr_loglevel(const char *buf, size_t count)
{
    int loglevel = 0;
    ...
    g_dev_loglevel[CLX_DRIVER_TYPES_CURR] = loglevel;
    return count;
}

static ssize_t clx_get_main_board_curr_debug(char *buf, size_t count)
{
    return sprintf(buf, "read curr sensor info: \n"
                        "cat /sys/s3ip/curr_sensor/curr*/*\n");
}

static ssize_t clx_set_main_board_curr_debug(const char *buf, size_t count)
{
    return -ENOSYS;
}
...
static struct s3ip_sysfs_curr_sensor_drivers_s drivers = {
     ...
     */
    .get_loglevel = clx_get_main_board_curr_loglevel,
    .set_loglevel = clx_set_main_board_curr_loglevel,
    .get_debug = clx_get_main_board_curr_debug,
    .set_debug = clx_set_main_board_curr_debug,  
