...
static ssize_t psu_debug_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    int ret;

    check_p(g_psu_drv);
    check_p(g_psu_drv->get_debug);

    ret = g_psu_drv->get_debug(buf, PAGE_SIZE);
    if (ret < 0) {
        PSU_ERR("get psu debug failed, ret: %d\n", ret);
        return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SYSFS_DEV_ERROR);
    }
    return ret;
}

static ssize_t psu_debug_store(struct switch_obj *obj, struct switch_attribute *attr,
                   const char* buf, size_t count)
{
    int ret;

    check_p(g_psu_drv);
    check_p(g_psu_drv->set_debug);
...
static ssize_t psu_loglevel_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    int ret;

    check_p(g_psu_drv);
    check_p(g_psu_drv->get_loglevel);

    ret = g_psu_drv->get_loglevel(buf, PAGE_SIZE);
    if (ret < 0) {
        PSU_ERR("get psu loglevel failed, ret: %d\n", ret);
        return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SYSFS_DEV_ERROR);
    }
    return ret;
}

static ssize_t psu_loglevel_store(struct switch_obj *obj, struct switch_attribute *attr,
                   const char* buf, size_t count)
{
    int ret;

    check_p(g_psu_drv);
    check_p(g_psu_drv->set_loglevel);
...
static ssize_t psu_out_min_vol_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    unsigned int psu_index;
    int ret;

    check_p(g_psu_drv);
    check_p(g_psu_drv->get_psu_out_min_vol);

    psu_index = obj->index;
    PSU_DBG("psu index: %u\n", psu_index);
    ret = g_psu_drv->get_psu_out_min_vol(psu_index, buf, PAGE_SIZE);
    if (ret < 0) {
        PSU_ERR("get psu%u output voltage failed, ret: %d\n", psu_index, ret);
        return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SYSFS_DEV_ERROR);
    }
...
static ssize_t psu_out_max_vol_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    unsigned int psu_index;
    int ret;

    check_p(g_psu_drv);
    check_p(g_psu_drv->get_psu_out_max_vol);

    psu_index = obj->index;
    PSU_DBG("psu index: %u\n", psu_index);
    ret = g_psu_drv->get_psu_out_max_vol(psu_index, buf, PAGE_SIZE);
    if (ret < 0) {
        PSU_ERR("get psu%u output voltage failed, ret: %d\n", psu_index, ret);
        return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SYSFS_DEV_ERROR);
    }
...
static ssize_t psu_date_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    unsigned int psu_index;
    int ret;
    ...
    psu_index = obj->index;
    PSU_DBG("psu index: %u\n", psu_index);
    ret = g_psu_drv->get_psu_date(psu_index, buf, PAGE_SIZE);
    if (ret < 0) {
        PSU_ERR("get psu%u date failed, ret: %d\n", psu_index, ret);
        return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SYSFS_DEV_ERROR);
    }
    return ret;
}

static ssize_t psu_vendor_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    unsigned int psu_index;
    int ret;
    ...
    psu_index = obj->index;
    PSU_DBG("psu index: %u\n", psu_index);
    ret = g_psu_drv->get_psu_vendor(psu_index, buf, PAGE_SIZE);
    if (ret < 0) {
        PSU_ERR("get psu%u vendor failed, ret: %d\n", psu_index, ret);
        return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SYSFS_DEV_ERROR);
    }
    return ret;
}

static ssize_t psu_alarm_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    unsigned int psu_index;
    int ret;
    ...
    psu_index = obj->index;
    PSU_DBG("psu index: %u\n", psu_index);
    ret = g_psu_drv->get_psu_alarm(psu_index, buf, PAGE_SIZE);
    if (ret < 0) {
        PSU_ERR("get psu%u alarm failed, ret: %d\n", psu_index, ret);
        return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SYSFS_DEV_ERROR);
    }
    return ret;
}

static ssize_t psu_alarm_threshold_curr_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    unsigned int psu_index;
    int ret;

    check_p(g_psu_drv);
    check_p(g_psu_drv->get_psu_alarm_threshold_curr);

    psu_index = obj->index;
    PSU_DBG("psu index: %u\n", psu_index);
    ret = g_psu_drv->get_psu_alarm_threshold_curr(psu_index, buf, PAGE_SIZE);
    if (ret < 0) {
        PSU_ERR("get psu%u alarm threshold current failed, ret: %d\n", psu_index, ret);
        return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SYSFS_DEV_ERROR);
    }
    return ret;
}

static ssize_t psu_alarm_threshold_vol_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    unsigned int psu_index;
    int ret;

    check_p(g_psu_drv);
    check_p(g_psu_drv->get_psu_alarm_threshold_vol);

    psu_index = obj->index;
    PSU_DBG("psu index: %u\n", psu_index);
    ret = g_psu_drv->get_psu_alarm_threshold_vol(psu_index, buf, PAGE_SIZE);
    if (ret < 0) {
        PSU_ERR("get psu%u alarm threshold voltage failed, ret: %d\n", psu_index, ret);
        return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SYSFS_DEV_ERROR);
    }
    return ret;
}

static ssize_t psu_max_output_power_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    unsigned int psu_index;
    int ret;

    check_p(g_psu_drv);
    check_p(g_psu_drv->get_psu_max_output_power);

    psu_index = obj->index;
    PSU_DBG("psu index: %u\n", psu_index);
    ret = g_psu_drv->get_psu_max_output_power(psu_index, buf, PAGE_SIZE);
    if (ret < 0) {
        PSU_ERR("get psu%u max output power failed, ret: %d\n", psu_index, ret);
        return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SYSFS_DEV_ERROR);
    }
    return ret;
}
                   ...
                   const char* buf, size_t count)
{
    ...
    if (ret < 0) {
        PSU_ERR("set psu%u temp%u max threshold failed, value: %s, count: %lu, ret: %d\n",
            psu_index, temp_index+1, buf, count, ret);
        return -EIO;
    }
...
static ssize_t psu_temp_max_hyst_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    unsigned int psu_index, temp_index;
    int ret;
    struct switch_obj *p_obj;
    ...
    check_p(g_psu_drv);
    check_p(g_psu_drv->get_psu_temp_max_hyst);
    ...
    PSU_DBG("psu index: %u, temp index: %u\n", psu_index, temp_index+1);
    ret = g_psu_drv->get_psu_temp_max_hyst(psu_index, temp_index, buf, PAGE_SIZE);
    if (ret < 0) {
        PSU_ERR("get psu%u temp%u max hyst threshold failed, ret: %d\n",
            psu_index, temp_index+1, ret);
        return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SYSFS_DEV_ERROR);
    }
    return ret;
...
static ssize_t psu_temp_max_hyst_store(struct switch_obj *obj, struct switch_attribute *attr,
                   const char* buf, size_t count)
{
    unsigned int psu_index, temp_index;
    int ret;
    ...
    check_p(g_psu_drv);
    check_p(g_psu_drv->set_psu_temp_max_hyst);

    p_obj = to_switch_obj(obj->kobj.parent);
    psu_index = p_obj->index;
    temp_index = obj->index;
    ret = g_psu_drv->set_psu_temp_max_hyst(psu_index, temp_index, buf, count);
    if (ret < 0) {
        PSU_ERR("set psu%u temp%u max hyst threshold failed, value: %s, count: %lu, ret: %d\n",
            psu_index, temp_index+1, buf, count, ret);
        return -EIO;
    }
    PSU_DBG("set psu%u temp%u max hyst threshold success, value: %s, count: %lu, ret: %d\n",
        psu_index, temp_index+1, buf, count, ret);
    return count;
}
...
/************************************psu dir and attrs*******************************************/
static struct switch_attribute psu_number_attr = __ATTR(number, S_IRUGO, psu_number_show, NULL);
static struct switch_attribute psu_debug_attr = __ATTR(debug, S_IRUGO | S_IWUSR, psu_debug_show, psu_debug_store);
static struct switch_attribute psu_loglevel_attr = __ATTR(loglevel, S_IRUGO | S_IWUSR, psu_loglevel_show, psu_loglevel_store);

static struct attribute *psu_dir_attrs[] = {
    &psu_number_attr.attr,
    &psu_debug_attr.attr,
    &psu_loglevel_attr.attr,
    NULL,
};
...
static struct switch_attribute psu_out_min_vol_attr = __ATTR(out_min_vol, S_IRUGO, psu_out_min_vol_show, NULL);
static struct switch_attribute psu_out_max_vol_attr = __ATTR(out_max_vol, S_IRUGO, psu_out_max_vol_show, NULL);
...
static struct switch_attribute psu_alarm_attr = __ATTR(alarm, S_IRUGO, psu_alarm_show, NULL);
...
static struct attribute *psu_attrs[] = {
    ...
    &psu_out_min_vol_attr.attr,
    &psu_out_max_vol_attr.attr,
...
static struct switch_attribute psu_temp_max_hyst_attr = __ATTR(max_hyst, S_IRUGO | S_IWUSR, psu_temp_max_hyst_show, psu_temp_max_hyst_store);
...
{
    struct temp_obj_s *curr_temp; /* point to temp0 temp1...*/
...
static int psuindex_single_temp_create_kobj_and_attrs(struct psu_obj_s *curr_psu, unsigned int temp_index)
{
    ...
    snprintf(name, sizeof(name), "temp%u", temp_index+1);
