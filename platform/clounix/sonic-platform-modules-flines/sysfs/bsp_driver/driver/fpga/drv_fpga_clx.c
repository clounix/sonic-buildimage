...
#define FPGA_POWER_HISTORY_STATUS (0x3b0)
#define FPGA_POWER_HISTORY_CTRL (0x3b4)
...
static ssize_t drv_get_main_board_fpga_power_history_reg(void *fpga, unsigned int fpga_index, char *buf, size_t count)
{
    struct fpga_driver_common *dev = (struct fpga_driver_common *)fpga;
    uint32_t data = 0;
    ...
    if (!dev || !dev->fpga_base)
        return -ENODEV;

    data = readl(dev->fpga_base + FPGA_POWER_HISTORY_STATUS);
    LOG_DBG(CLX_DRIVER_TYPES_FPGA, " reg: %x, data: 0x%x\n", FPGA_POWER_HISTORY_STATUS, data);

    return scnprintf(buf, count, "0x%x\n", data);
}

static ssize_t drv_get_main_board_fpga_power_history_ctrl(void *fpga, unsigned int fpga_index, char *buf, size_t count)
{
    struct fpga_driver_common *dev = (struct fpga_driver_common *)fpga;
    uint32_t data = 0;
    ...
    data = readl(dev->fpga_base + FPGA_POWER_HISTORY_CTRL);
    LOG_DBG(CLX_DRIVER_TYPES_FPGA, " reg: %x, data: 0x%x\n", FPGA_POWER_HISTORY_CTRL, data);

    return scnprintf(buf, count, "0x%x\n", data);
}

/*
 * drv_set_main_board_fpga_power_history_ctrl - Set power history control bit
 * @fpga_index: index of FPGA (unused in current impl)
 * @value: 0 or 1
 *
 * Returns 0 on success, negative error code on failure.
 */
static int drv_set_main_board_fpga_power_history_ctrl(void *fpga, unsigned int fpga_index, unsigned int value)
{
    struct fpga_driver_common *dev = (struct fpga_driver_common *)fpga;
    uint32_t data = 0;
    ...
    writel(data, dev->fpga_base + FPGA_POWER_HISTORY_CTRL);
...
int clx_driver_fpga_common_init(void **fpga_driver)
{
    ...
    fpga->fpga_if.get_main_board_fpga_power_history_reg = drv_get_main_board_fpga_power_history_reg;
    fpga->fpga_if.get_main_board_fpga_power_history_ctrl = drv_get_main_board_fpga_power_history_ctrl;
    fpga->fpga_if.set_main_board_fpga_power_history_ctrl = drv_set_main_board_fpga_power_history_ctrl;
