...
extern int board_i2c_fpga_read(unsigned short fpga_addr, u8 reg);
extern int board_i2c_fpga_write(unsigned short fpga_addr, u8 reg, u8 value);
...
PDDF_DATA_ATTR(r_cpld3_eeprom_wp, S_IWUSR|S_IRUGO, show_reg_data, store_reg_data, PDDF_UCHAR, sizeof(uint32_t), NULL, NULL);
...
PDDF_DATA_ATTR(r_fpga1_power_good, S_IRUGO, show_reg_data, NULL, PDDF_UCHAR, sizeof(uint32_t), NULL, NULL);
PDDF_DATA_ATTR(r_fpga1_power_status, S_IRUGO, show_reg_data, NULL, PDDF_UCHAR, sizeof(uint32_t), NULL, NULL);
PDDF_DATA_ATTR(r_fpga1_power_err_clr, S_IWUSR|S_IRUGO, show_reg_data, store_reg_data, PDDF_UCHAR, sizeof(uint32_t), NULL, NULL);
PDDF_DATA_ATTR(r_fpga1_power_err_triggered, S_IRUGO, show_reg_data, NULL, PDDF_UCHAR, sizeof(uint32_t), NULL, NULL);
PDDF_DATA_ATTR(r_fpga1_power_err_current, S_IRUGO, show_reg_data, NULL, PDDF_UCHAR, sizeof(uint32_t), NULL, NULL);
PDDF_DATA_ATTR(r_fpga1_power_timeout_state, S_IRUGO, show_reg_data, NULL, PDDF_UCHAR, sizeof(uint32_t), NULL, NULL);
PDDF_DATA_ATTR(r_fpga1_pvt_err_clr, S_IWUSR|S_IRUGO, show_reg_data, store_reg_data, PDDF_UCHAR, sizeof(uint32_t), NULL, NULL);
PDDF_DATA_ATTR(r_fpga1_pvt_err_triggered, S_IRUGO, show_reg_data, NULL, PDDF_UCHAR, sizeof(uint32_t), NULL, NULL);
PDDF_DATA_ATTR(r_fpga1_pvt_err_current, S_IRUGO, show_reg_data, NULL, PDDF_UCHAR, sizeof(uint32_t), NULL, NULL);
...
static struct attribute *sysstatus_data_attributes[] = {
    ...
    &attr_r_cpld3_eeprom_wp.dev_attr.attr,
    ...
    &attr_r_fpga1_power_good.dev_attr.attr,
    &attr_r_fpga1_power_status.dev_attr.attr,
    &attr_r_fpga1_power_err_clr.dev_attr.attr,
    &attr_r_fpga1_power_err_triggered.dev_attr.attr,
    &attr_r_fpga1_power_err_current.dev_attr.attr,
    &attr_r_fpga1_power_timeout_state.dev_attr.attr,
    &attr_r_fpga1_pvt_err_clr.dev_attr.attr,
    &attr_r_fpga1_pvt_err_triggered.dev_attr.attr,
    &attr_r_fpga1_pvt_err_current.dev_attr.attr,
    NULL
};
...
ssize_t show_reg_data(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    else
    {
        ...
        if (strncmp(sysstatus_addr_attrs->devtype, "fpgai2c", strlen("fpgai2c")) == 0)
        {
            status = board_i2c_fpga_read( sysstatus_addr_attrs->devaddr, sysstatus_addr_attrs->offset);
            pddf_dbg(SYSSTATUS,  "%s: reg: 0x%x,value: 0x%x\n", __FUNCTION__, sysstatus_addr_attrs->offset,status);
        }
...
ssize_t store_reg_data(struct device *dev, struct device_attribute *da, const char *buf, size_t count)
{
    ...
    else
    {
        ...
        {
            cpld_reg_val = cpld_reg_val&sysstatus_addr_attrs->mask;
            status = board_i2c_fpga_write( sysstatus_addr_attrs->devaddr, sysstatus_addr_attrs->offset, cpld_reg_val);
            pddf_dbg(SYSSTATUS,  "%s: cpld reg: 0x%x,value: 0x%x\n", __FUNCTION__, sysstatus_addr_attrs->offset, cpld_reg_val);
        }
    }
    return count;
