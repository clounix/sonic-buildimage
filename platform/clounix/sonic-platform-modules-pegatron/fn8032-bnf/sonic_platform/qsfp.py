...
class QSfp(SfpBase):
    ...
    def __init__(self, index):
        ...
        self.__platform = "x86_64-pegatron_fn8032-bnf-r0"
        self.__hwsku    = "fn8032-bnf"
        ...
        if self.__index in range(0, self.__port_end + 1):
            ...
            if self.__index < self.cpldb_sfp_num:
                reset_path = '/sys/bus/i2c/devices/{0}-0075/sfp'+str(self.__index+1)+'_reset'
            else:
                reset_path = '/sys/bus/i2c/devices/{0}-0076/sfp'+str(self.__index+1)+'_reset'
            self.__reset_attr = reset_path.format(self.__port_to_i2c_mapping[self.__index])
            if self.__index < self.cpldb_sfp_num:
                lowpower_path = '/sys/bus/i2c/devices/{0}-0075/sfp'+str(self.__index+1)+'_lowpower'
            else:
                lowpower_path = '/sys/bus/i2c/devices/{0}-0076/sfp'+str(self.__index+1)+'_lowpower'
            self.__lpmode_attr = lowpower_path.format(self.__port_to_i2c_mapping[self.__index])
