...
static int *log_level = &lpc_log_level;
...
unsigned char lpc_cpld_read_reg(u16 address)
{
    ...
    pddf_dbg(LPC, "cpld_base:0x%x, address:0x%x, value:0x%02x\n", CPLD_LPC_BASE, address, reg_val);
...
static int __init pddf_lpc_cpld_init(void)
{
     ...
     */
        ...
        if(status)
        { 
            pddf_err(LPC, "pci_write_config_word error %d!\n",status);
            return status;
        } 
    } else {
        ...
        if (pdev) {
            ...
            if(status)
            { 
                pddf_err(LPC, "pci_write_config_byte error %d!\n",status);
                return status;
            }  
        } else {
            pddf_info(LPC, "there is no LPC controller\n");
            return 0;
        }

    }
    if (!request_region(CPLD_LPC_BASE, CPLD_LPC_SIZE, "lpc_sys_cpld")) {
        pddf_err(LPC, "request_region 0x%x failed!\n", CPLD_LPC_BASE);
		return -EBUSY;
	}
    return 0; 
}
...
static int cpld_lpc_drv_probe(struct platform_device *pdev)
{
    ...
    pddf_dbg(LPC, "Enable LPC config registers successful\n");
    res = platform_get_resource(pdev, IORESOURCE_MEM, 0);
    if (unlikely(!res)) {
        pddf_err(LPC, "Specified Resource Not Available...\n");
        return -ENODEV;
    }
	...
	if (!pdata->lpc_base)
	{
        pddf_err(LPC, "@%p: Unable to map LPC  registers\n", res->start);
        return -ENOMEM;
    }
    g_cpld_lpc_base = pdata->lpc_base;
    pddf_info(LPC, "@%p:  map LPC  registers succsess\n", pdata->lpc_base);
