...
typedef enum knet_intr_flags_e {
    INTR_FLAGS_DEINIT = 0,
    INTR_FLAGS_INIT = 1,
} knet_intr_flags_t;
...
typedef void (*unregister_msi_irq)(uint32_t unit);
typedef int (*dma_channel_error_irq_handle)(uint32_t unit, uint32_t channel);
...
typedef struct {
    ...
    dma_channel_error_irq_handle dma_channel_error_irq_handle;

    usr_interrupt_irq usr_interrupt_exist;

    register_msi_irq register_msi_irq;
    unregister_msi_irq unregister_msi_irq;
} clx_intr_drv_cb_t;
...
/* init */
int
clx_interrupt_init(uint32_t unit, uint32_t intr_mode);
void
