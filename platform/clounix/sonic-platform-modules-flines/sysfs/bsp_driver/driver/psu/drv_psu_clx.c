...
static DEFINE_MUTEX(psu_list_mutex);
...
#define DIR_IN "_input"
#define DIR_OUT "out"
#define DIR_OUT_MIN "_rated_min"
#define DIR_OUT_MAX "_rated_max"
...
static struct i2c_client *client_arry[MAX_PSU_DATA_NUM + 1] = {NULL};
static struct device *client_priv_arry[MAX_PSU_DATA_NUM + 1] = {NULL};
...
static ssize_t drv_get_psu_in_curr(void *driver, unsigned int psu_index, char *buf, size_t count)
{
    ...
    /* add vendor codes here */
    INDEX_CHECK(psu_index);
    memset(node_name, 0x00, PMBUS_NAME_SIZE);
    mutex_lock(&psu_list_mutex);
    client = client_arry[psu_index];
    if (client != NULL)
    {
        snprintf(node_name, sizeof(node_name), "%s%d%s", CURR_CLASS, 1, DIR_IN);
...
static ssize_t drv_get_psu_in_vol(void *driver, unsigned int psu_index, char *buf, size_t count)
{
    ...
    if (client != NULL)
    {
        snprintf(node_name, sizeof(node_name), "%s%d%s", VOL_CLASS, 1, DIR_IN);
...
static ssize_t drv_get_psu_in_power(void *driver, unsigned int psu_index, char *buf, size_t count)
{
    ...
    if (client != NULL)
    {
        snprintf(node_name, sizeof(node_name), "%s%d%s", POWER_CLASS, 1, DIR_IN);
...
static ssize_t drv_get_psu_out_curr(void *driver, unsigned int psu_index, char *buf, size_t count)
{
    ...
    if (client != NULL)
    {
        snprintf(node_name, sizeof(node_name), "%s%d%s", CURR_CLASS, 2, DIR_IN);
...
static ssize_t drv_get_psu_out_vol(void *driver, unsigned int psu_index, char *buf, size_t count)
{
    ...
    if (client != NULL)
    {
        snprintf(node_name, sizeof(node_name), "%s%d%s", VOL_CLASS, 2, DIR_IN);
...
static ssize_t drv_get_psu_out_min_vol(void *driver, unsigned int psu_index, char *buf, size_t count)
{
    int ret = 0;
    struct i2c_client *client;
    char node_name[PMBUS_NAME_SIZE];
    ...
    if (client != NULL)
    {
        snprintf(node_name, sizeof(node_name), "%s%d%s", VOL_CLASS, 2, DIR_OUT_MIN);
...
static ssize_t drv_get_psu_out_max_vol(void *driver, unsigned int psu_index, char *buf, size_t count)
{
    int ret = 0;
    struct i2c_client *client;
    char node_name[PMBUS_NAME_SIZE];
    ...
    if (client != NULL)
    {
        snprintf(node_name, sizeof(node_name), "%s%d%s", VOL_CLASS, 2, DIR_OUT_MAX);
...
static ssize_t drv_get_psu_out_power(void *driver, unsigned int psu_index, char *buf, size_t count)
{
    ...
    if (client != NULL)
    {
        snprintf(node_name, sizeof(node_name), "%s%d%s", POWER_CLASS, 2, DIR_IN);
...
static ssize_t drv_get_psu_present_status(void *driver, unsigned int psu_index, char *buf, size_t count)
{
    ...
    if (dev != NULL)
    {
        snprintf(node_name, sizeof(node_name), "psu_%d%s", psu_index, PRST);
        ret = get_priv_attr_val_by_name(dev, node_name, buf);
        if (ret > 0)
            prst = *buf - '0';

        snprintf(node_name, sizeof(node_name), "psu_%d%s", psu_index, ACOK);
        ret = get_priv_attr_val_by_name(dev, node_name, buf);
        if (ret > 0)
            acok = *buf - '0';
    }
...
static ssize_t drv_get_psu_out_status(void *driver, unsigned int psu_index, char *buf, size_t count)
{
    ...
    if (dev != NULL)
    {
        snprintf(node_name, sizeof(node_name), "psu_%d%s", psu_index, PWOK);
        ret = get_priv_attr_val_by_name(dev, node_name, buf);
    }
    mutex_unlock(&psu_list_mutex);
...
static ssize_t drv_get_psu_fan_speed(void *driver, unsigned int psu_index, char *buf, size_t count)
{
    ...
    if (client != NULL)
    {
        snprintf(node_name, sizeof(node_name), "%s%d%s", FAN_CLASS, 1, OUTPUT_NODE);
...
static ssize_t drv_get_psu_fan_ratio(void *driver, unsigned int psu_index, char *buf, size_t count)
{
    ...
    if (client != NULL)
    {
        snprintf(node_name, sizeof(node_name), "%s%d%s", FAN_CLASS, 1, FAN_RATIO);
        ret = get_attr_val_by_name(client, node_name, buf);
    }
    mutex_unlock(&psu_list_mutex);
                                ...
                                const char *buf, size_t count)
{
    ...
    if (client != NULL)
    {
        snprintf(node_name, sizeof(node_name), "%s%d%s", TEMP_CLASS, temp_index, TEMP_MAX);
                                    ...
                                    char *buf, size_t count)
{
    ...
    if (client != NULL)
    {
        snprintf(node_name, sizeof(node_name), "%s%d%s", TEMP_CLASS, temp_index, TEMP_MIN);
        ret = get_attr_val_by_name(client, node_name, buf);
    }
    mutex_unlock(&psu_list_mutex);
                                      ...
                                      char *buf, size_t count)
{
    ...
    {
        temp_index += PSU_TEMP_INDEX_OFFSET;
        snprintf(node_name, sizeof(node_name), "%s%d%s", TEMP_CLASS, temp_index, OUTPUT_NODE);
        ret = get_attr_val_by_name(client, node_name, buf);
    }
    mutex_unlock(&psu_list_mutex);
                                         ...
                                         char *buf, size_t count)
{
    ...
    if (client != NULL)
    {
        snprintf(node_name, sizeof(node_name), "%s%d%s", TEMP_CLASS, temp_index, TEMP_MAX_HYST);
        ret = get_attr_val_by_name(client, node_name, buf);
    }
    mutex_unlock(&psu_list_mutex);
...
int drv_psu_clx_init(void **psu_driver)
{
    ...
    psu->psu_if.get_psu_out_min_vol = drv_get_psu_out_min_vol;
    psu->psu_if.get_psu_out_max_vol = drv_get_psu_out_max_vol;
