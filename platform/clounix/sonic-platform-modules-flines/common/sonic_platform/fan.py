...
class Fan(FanBase):
    ...
    def __init__(self, index,fan_conf):
        ...
        if fan_conf[self.__index]['container'] == 'psu':
            self.__attr_path_prefix = '/sys/s3ip/psu/psu{}/'.format(fan_conf[self.__index]['container_index'] + 1)
            self.__is_psu_fan = True
        elif fan_conf[self.__index]['container'] == 'fan_drawer':
            self.__attr_path_prefix = '/sys/s3ip/fan/fan{}/'.format(self.__index+1)
        else:
...
##############################################
    ...
    def get_speed(self):
        ...
        if self.__is_psu_fan :
            ...
            attr_rv = self.__api_helper.read_one_line_file(self.__attr_path_prefix + 'motor1/speed')
        if (attr_rv != None):
            attr_rv = int(attr_rv) 
            if (attr_rv >= 0):
                speed = attr_rv
    ...
    def get_target_speed(self):
        ...
        if self.__is_psu_fan :
            ...
            attr_rv = self.__api_helper.read_one_line_file(self.__attr_path_prefix + 'ratio')
            speed_max = self.__api_helper.read_one_line_file(self.__attr_path_prefix + 'motor1/speed_max')
            if (speed_max != None):
    ...
    def set_speed(self, speed):
        ...
        if not self.__is_psu_fan and self.get_presence():            
            return self.__api_helper.write_txt_file(self.__attr_path_prefix + 'ratio', int(speed))
        return False
