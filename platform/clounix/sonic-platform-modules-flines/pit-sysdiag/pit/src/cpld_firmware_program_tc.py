...
MASTER_ORIG_FW_CFG = "cpld_master_firmware_baseline.json"
SLAVE_ORIG_FW_CFG = "cpld_slave_firmware_baseline.json"


class CPLDFIRMWAREPROGRAMTC(TestCaseCommon):
    ...
    def __init__(self, index, logger, platform_cfg_file, case_cfg_file=None):
        ...
        MODULE_NAME = "cpld_firmware_program_tc"
        ...
        try:
            self.firmware_type = self.platform_cfg_json["cpld_firmware_type"]
        ...
        try:
            for item in self.firmware_type:
                ...
                if not result:
                    ...
                    ret = False
                    break
                result = eval(self.firmware_upgrade_func)(item, fw_path, self.fw_extra, self.platform_path)
                if not result:
                    self.fail_reason.append(
                        "upgrade %s %s %s firmware fail"
                        % (item, self.fw_extra, test_type)
