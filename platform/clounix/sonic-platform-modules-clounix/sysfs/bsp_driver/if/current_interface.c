...
int current_if_create_driver(void)
{
    ...
    if (rc == DRIVER_OK) {
        ...
        current_driver->total_sensor_node = bd->curr.total_sensor_node;
        current_driver->real_max_sensor_num = bd->curr.real_max_sensor_num;
        current_driver->sensor_map = bd->curr.sensor_map;
        current_driver->index_range_map = bd->curr.index_range_map;
    }
