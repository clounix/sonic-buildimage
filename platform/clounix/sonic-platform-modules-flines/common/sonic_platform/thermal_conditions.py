...
class ThermalOverHighCondition(ThermalCondition):
    def is_match(self, thermal_info_dict):
...
class AsicCondition(ThermalPolicyConditionBase):
    def get_asic_info(self, thermal_info_dict):
        from .thermal_infos import AsicInfo
        if AsicInfo.INFO_NAME in thermal_info_dict and isinstance(thermal_info_dict[AsicInfo.INFO_NAME], AsicInfo):
            return thermal_info_dict[AsicInfo.INFO_NAME]
        else:
            return None


@thermal_json_object('asic.over.high_threshold')
class AsicOverHighCondition(AsicCondition):
    def is_match(self, thermal_info_dict):
        asic_info_obj = self.get_asic_info(thermal_info_dict)
        if asic_info_obj:
            return asic_info_obj.is_over_high_threshold()
        else:
            return False


@thermal_json_object('asic.over.high_critical_threshold')
class AsicOverHighCriticalCondition(AsicCondition):
    def is_match(self, thermal_info_dict):
        asic_info_obj = self.get_asic_info(thermal_info_dict)
        if asic_info_obj:
            return asic_info_obj.is_over_high_critical_threshold()
        else:
            return False


@thermal_json_object('asic.all.good')
class AsicGoodCondition(AsicCondition):
    def is_match(self, thermal_info_dict):
        asic_info_obj = self.get_asic_info(thermal_info_dict)
        if asic_info_obj:
            return not asic_info_obj.is_over_threshold()
        else:
            return False
