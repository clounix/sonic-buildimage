...
class Thermal(ThermalBase):
    ...
    def __init__(self, thermal_index, thermal_conf):
        ...
        if self.__conf[self.__index]['container'] == 'psu':
            self.__attr_path_prefix = '/sys/switch/psu/psu{}/temp{}/'.format(
                self.__conf[self.__index]['container_index']+1, self.__index)
        else:
            self.__attr_path_prefix = '/sys/switch/sensor/temp{}/'.format(
                self.__index)
    ...
    def get_high_threshold(self):
        ...
        attr_rv = self.__api_helper.read_one_line_file(
            self.__attr_path_prefix + 'temp_max_hyst')
        if ((attr_rv != None) and (attr_rv != 'NA')):
            attr_rv = int(attr_rv, 10)
    ...
    def set_high_threshold(self, temperature):
        ...
        is_set = self.__api_helper.write_txt_file(
            self.__attr_path_prefix + 'temp_max_hyst', int(temperature*1000))
        return is_set
    ...
    def get_high_critical_threshold(self):
        ...
        attr_rv = self.__api_helper.read_one_line_file(
            self.__attr_path_prefix + 'temp_max')
        if ((attr_rv != None) and (attr_rv != 'NA')):
            attr_rv = int(attr_rv, 10)
            high_threshold = float(attr_rv/1000)
        return high_threshold
