    ...
    if (info!=NULL)
    {
        ...
        if (client_ptr)
        {
            ...
            else if (info->len==2)
            {
                ...
                pddf_err(XCVR, "Doesn't support block CPLD read yet");
        }
        else
...
int xcvr_i2c_cpld_write(XCVR_ATTR *info, uint32_t val)
{
    ...
    else
    {
        ...
        {
            pddf_err(XCVR, "Doesn't support block CPLD write yet");
            status = -1;
        }
    }
    return status;
    ...
    if (info!=NULL)
    {
        ...
        if (client_ptr)
        {
            ...
            else if (info->len==2)
            {
                ...
                pddf_err(XCVR, "Doesn't support block FPGAI2C read yet");
        }
        else
            pddf_err(XCVR, "Unable to get the client handle for %s\n", info->devname);
    }
...
int xcvr_i2c_fpga_write(XCVR_ATTR *info, uint32_t val)
{
    ...
    else
    {
        ...
        {
            pddf_err(XCVR, "Doesn't support block FPGAI2C write yet");
            status = -1;
        }
    }
    return status;
...
int xcvr_multifpgapci_read(XCVR_ATTR *info, int *output)
{
    ...
    if (ptr_multifpgapci_readpci == NULL) {
        pddf_err(XCVR, "PDDF_XCVR: Doesn't support MULTIFPGAPCI read yet");
        status = -1;
        goto ret;
    }
    ...
    if (pci_dev == NULL) {
        pddf_err(XCVR, "PDDF_XCVR: Unable to get pci_dev of %s for %s\n", info->devname, info->aname);
        status = -1;
        goto ret;
    }
...
ret:
    if (status)
        pddf_err(XCVR, "%s: Error status = %d", __FUNCTION__, status);
...
int sonic_i2c_get_mod_power_en(struct i2c_client *client, XCVR_ATTR *info, struct xcvr_data *data)
{
    int status = 0;
    uint32_t power_en = 0;
    ...
    data->power_en = power_en;
    return 0;
}

int sonic_i2c_get_mod_power_fault(struct i2c_client *client, XCVR_ATTR *info, struct xcvr_data *data)
{
    int status = 0;
    uint32_t power_fault = 0;
...
int sonic_i2c_get_mod_overwrite_en(struct i2c_client *client, XCVR_ATTR *info, struct xcvr_data *data)
{
    int status = 0;
    uint32_t overwrite_en = 0;
    ...
    data->overwrite_en = overwrite_en;
    return 0;
}
...
int sonic_i2c_set_mod_reset(struct i2c_client *client, XCVR_ATTR *info, struct xcvr_data *data)
{
    ...
    {
        pddf_err(XCVR, "Error: Invalid device type (%s) to set xcvr reset\n", info->devtype);
        status = -1;
    }
...
int sonic_i2c_set_mod_lpmode(struct i2c_client *client, XCVR_ATTR *info, struct xcvr_data *data)
{
    ...
    {
        pddf_err(XCVR, "Error: Invalid device type (%s) to set xcvr lpmode\n", info->devtype);
        status = -1;
    }
...
int sonic_i2c_set_mod_power_en(struct i2c_client *client, XCVR_ATTR *info, struct xcvr_data *data)
{
    int status = 0;

    if (strcmp(info->devtype, "cpld") == 0)
    {
        status = xcvr_i2c_cpld_write(info, data->power_en);
    }
    else if (strcmp(info->devtype, "fpgai2c") == 0)
    {
        status = xcvr_i2c_fpga_write(info, data->power_en);
    }
    else if (strcmp(info->devtype, "fpgapci") == 0)
    {
        status = xcvr_fpgapci_write(info, data->power_en);
    }
    else
    {
        pddf_err(XCVR, "Error: Invalid device type (%s) to set xcvr power_en\n", info->devtype);
        status = -1;
    }
...
int sonic_i2c_set_mod_txdisable(struct i2c_client *client, XCVR_ATTR *info, struct xcvr_data *data)
{
    ...
    {
        pddf_err(XCVR, "Error: Invalid device type (%s) to set xcvr txdisable\n", info->devtype);
        status = -1;
    }
...
int sonic_i2c_set_mod_overwrite_en(struct i2c_client *client, XCVR_ATTR *info, struct xcvr_data *data)
{
    int status = 0;

    if (strcmp(info->devtype, "cpld") == 0)
    {
        status = xcvr_i2c_cpld_write(info, data->overwrite_en);
    }
    else if (strcmp(info->devtype, "fpgai2c") == 0)
    {
        status = xcvr_i2c_fpga_write(info, data->overwrite_en);
    }
    else if (strcmp(info->devtype, "fpgapci") == 0)
    {
        status = xcvr_fpgapci_write(info, data->overwrite_en);
    }
    else
    {
        pddf_err(XCVR, "Error: Invalid device type (%s) to set xcvr overwrite_en\n", info->devtype);
        status = -1;
    }
...
ssize_t get_module_power_en(struct device *dev, struct device_attribute *da, char *buf)
{
    struct sensor_device_attribute *attr = to_sensor_dev_attr(da);
    struct i2c_client *client = to_i2c_client(dev);
    XCVR_PDATA *pdata = (XCVR_PDATA *)(client->dev.platform_data);
...
ssize_t set_module_power_en(struct device *dev, struct device_attribute *da, const char *buf, 
        size_t count)
{
    struct sensor_device_attribute *attr = to_sensor_dev_attr(da);
    struct i2c_client *client = to_i2c_client(dev);
...
ssize_t get_module_power_fault(struct device *dev, struct device_attribute *da, char *buf)
{
    struct sensor_device_attribute *attr = to_sensor_dev_attr(da);
    struct i2c_client *client = to_i2c_client(dev);
    XCVR_PDATA *pdata = (XCVR_PDATA *)(client->dev.platform_data);
...
ssize_t get_module_overwrite_en(struct device *dev, struct device_attribute *da, char *buf)
{
    struct sensor_device_attribute *attr = to_sensor_dev_attr(da);
    struct i2c_client *client = to_i2c_client(dev);
    XCVR_PDATA *pdata = (XCVR_PDATA *)(client->dev.platform_data);
...
ssize_t set_module_overwrite_en(struct device *dev, struct device_attribute *da, const char *buf, 
        size_t count)
{
    struct sensor_device_attribute *attr = to_sensor_dev_attr(da);
    struct i2c_client *client = to_i2c_client(dev);
