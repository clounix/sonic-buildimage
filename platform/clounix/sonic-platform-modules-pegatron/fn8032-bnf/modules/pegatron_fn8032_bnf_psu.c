...
static int pega_fn8032_bnf_psu_read_block(unsigned short addr, u8 reg,u8* buf)
{
    struct list_head   *list_node = NULL;
    struct psu_client_node *psu_node = NULL;
...
static ssize_t read_psu_alarm(struct device *dev, struct device_attribute *da,
             char *buf)
{
    struct fn8032_bnf_psu_data *drv_data =  dev_get_drvdata(dev);
    struct i2c_client *client = drv_data->client;    
    u8 vout_data = 0, temp_data = 0,fan_data = 0,val = 0;

    vout_data = pega_fn8032_bnf_psu_read(client->addr, PSU_VOUT_STATUS_REG);
    temp_data = pega_fn8032_bnf_psu_read(client->addr, PSU_TEMP_STATUS_REG);
    fan_data = pega_fn8032_bnf_psu_read(client->addr, PSU_FANS_1_2_STATUS);
    pega_print(DEBUG, "vout_data:0x%x,temp_data:0x%x,fan_data:0x%x\r\n", vout_data,temp_data,fan_data);
    
    //BIT0:temprature status,BIT1:fan status,BIT2:vol status
    if(fan_data & 0xff) 
        SET_BIT(val,0);
    if(temp_data & 0xff)
        SET_BIT(val,1);
                           ...
                           char *buf)
{
    ...
    read_value = pega_fn8032_bnf_psu_read_word(client->addr, psu_pmbus_reg_map[attr->index]);
                                ...
                                char *buf)
{
    ...
    data = pega_fn8032_bnf_psu_read_word(client->addr, psu_pmbus_reg_map[attr->index]);
...
static ssize_t pega_fn8032_bnf_psu_read_data_direct(struct device *dev, struct device_attribute *da,
                                                    char *buf)
{
    struct sensor_device_attribute *attr = to_sensor_dev_attr(da);
    struct fn8032_bnf_psu_data *drv_data = dev_get_drvdata(dev);
                                     ...
                                     char *buf)
{
    ...
    read_count = pega_fn8032_bnf_psu_read_block(client->addr, psu_pmbus_reg_map[attr->index], data);
    data[read_count] = '\0';
                                          ...
                                          struct device_attribute *da, char *buf)
{
    ...
    read_value = pega_fn8032_bnf_psu_read_word(client->addr, reg);
    exponent = two_complement_to_int(read_value >> 11, 5, 0x1f);
    mantissa = two_complement_to_int(read_value & 0x7ff, 11, 0x7ff);
                                 ...
                                 char *buf)
{
    struct fn8032_bnf_psu_data *drv_data = dev_get_drvdata(dev);
    if (!drv_data) {
        pega_print(ERR, "fn8032_bnf_psu_data is NULL!\n");
        return -ENOENT;
    }
...
static SENSOR_DEVICE_ATTR(power_in_max,S_IRUGO, pega_fn8032_bnf_psu_read_data_direct, NULL, PSU_PIN_MAX);
...
static SENSOR_DEVICE_ATTR(power_out_max,S_IRUGO, pega_fn8032_bnf_psu_read_data_direct, NULL, PSU_POUT_MAX);
...
#define SET_TEMP_ATTR(_num) \
    ...
    static SENSOR_DEVICE_ATTR(temp##_num##_max,  S_IRUGO | S_IWUSR, get_psu_sensor_temp_max, set_psu_sensor_temp_max, _num-1); \
    static SENSOR_DEVICE_ATTR(temp##_num##_crit,  S_IRUGO | S_IWUSR, get_psu_sensor_temp_crit, set_psu_sensor_temp_crit, _num-1);

SET_TEMP_ATTR(1);SET_TEMP_ATTR(2);SET_TEMP_ATTR(3);

static struct attribute *pega_fn8032_bnf_psu_attrs[] = {
...
ATTRIBUTE_GROUPS(pega_fn8032_bnf_psu);
                                     ...
                                     const struct i2c_device_id *dev_id)
{
    ...
    struct fn8032_bnf_psu_data *data;
    ...
    data = devm_kzalloc(dev, sizeof(struct fn8032_bnf_psu_data), GFP_KERNEL);
    if (data == NULL)
        return -ENOMEM;
    data->client = client;
    ...
    for (i = 0; i < MAX_PSU_TEMP_SENSORS; i++) {
        data->temp_max[i] = 50*1000;
        data->temp_crit[i] = 60*1000;
    }
    ...
    case PSU_59_ADDRESS:
        ...
        hwmon_dev = devm_hwmon_device_register_with_groups(dev, client->name,
                                                           data, pega_fn8032_bnf_psu_groups);
        if (IS_ERR(hwmon_dev))
