...
struct s3ip_sysfs_fpga_drivers_s {
    ...
    ssize_t (*get_main_board_fpga_power_history_reg)(unsigned int fpga_index, char *buf, size_t count);
    ssize_t (*get_main_board_fpga_power_history_ctrl)(unsigned int fpga_index, char *buf, size_t count);
    int (*set_main_board_fpga_power_history_ctrl)(unsigned int fpga_index, unsigned int value);
