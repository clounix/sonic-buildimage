...
static ssize_t curr_sensor_debug_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    int ret;

    check_p(g_curr_sensor_drv);
    check_p(g_curr_sensor_drv->get_debug);

    ret = g_curr_sensor_drv->get_debug(buf, PAGE_SIZE);
    if (ret < 0)
    {
        CURR_SENSOR_ERR("get sensor debug failed, ret: %d\n", ret);
        return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SYSFS_DEV_ERROR);
    }
    return ret;
}

static ssize_t curr_sensor_debug_store(struct switch_obj *obj, struct switch_attribute *attr,
                                  const char *buf, size_t count)
{
    int ret;

    check_p(g_curr_sensor_drv);
    check_p(g_curr_sensor_drv->set_debug);

    ret = g_curr_sensor_drv->set_debug(buf, PAGE_SIZE);
    if (ret < 0)
    {
        CURR_SENSOR_ERR("set sensor debug failed, ret: %d\n", ret);
    }
    return ret;
}

static ssize_t curr_sensor_loglevel_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    int ret;

    check_p(g_curr_sensor_drv);
    check_p(g_curr_sensor_drv->get_loglevel);

    ret = g_curr_sensor_drv->get_loglevel(buf, PAGE_SIZE);
    if (ret < 0)
    {
        CURR_SENSOR_ERR("get sensor loglevel failed, ret: %d\n", ret);
        return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SYSFS_DEV_ERROR);
    }
    return ret;
}

static ssize_t curr_sensor_loglevel_store(struct switch_obj *obj, struct switch_attribute *attr,
                                     const char *buf, size_t count)
{
    int ret;

    check_p(g_curr_sensor_drv);
    check_p(g_curr_sensor_drv->set_loglevel);

    ret = g_curr_sensor_drv->set_loglevel(buf, PAGE_SIZE);
    if (ret < 0)
    {
        CURR_SENSOR_ERR("set sensor loglevel failed, ret: %d\n", ret);
    }
    return ret;
}
...
static ssize_t curr_sensor_crit_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    unsigned int curr_index;
    int ret;

    check_p(g_curr_sensor_drv);
    check_p(g_curr_sensor_drv->get_main_board_curr_crit);

    curr_index = obj->index;
    CURR_SENSOR_DBG("curr index: %u\n", curr_index+1);
    ret = g_curr_sensor_drv->get_main_board_curr_crit(curr_index, buf, PAGE_SIZE);
    if (ret < 0) {
        CURR_SENSOR_ERR("get curr%u crit failed, ret: %d\n", curr_index+1, ret);
        return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SYSFS_DEV_ERROR);
    }
    return ret;
}

static ssize_t curr_sensor_crit_store(struct switch_obj *obj, struct switch_attribute *attr,
                   const char* buf, size_t count)
{
    unsigned int curr_index;
    int ret;
    ...
    check_p(g_curr_sensor_drv);
    check_p(g_curr_sensor_drv->set_main_board_curr_crit);

    curr_index = obj->index;
    CURR_SENSOR_DBG("curr index: %u\n", curr_index+1);
    ret = g_curr_sensor_drv->set_main_board_curr_crit(curr_index, buf, count);
    if (ret < 0) {
        CURR_SENSOR_ERR("set curr%u crit failed, value: %s, count: %lu, ret: %d\n",
            curr_index+1, buf, count, ret);
        return -EIO;
    }
    CURR_SENSOR_DBG("set curr%u crit success, value: %s, count: %lu, ret: %d\n",
        curr_index+1, buf, count, ret);
    return count;
}

static ssize_t curr_sensor_average_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    unsigned int curr_index;
    int ret;

    check_p(g_curr_sensor_drv);
    check_p(g_curr_sensor_drv->get_main_board_curr_average);

    curr_index = obj->index;
    CURR_SENSOR_DBG("curr index: %u\n", curr_index+1);
    ret = g_curr_sensor_drv->get_main_board_curr_average(curr_index, buf, PAGE_SIZE);
    if (ret < 0) {
        CURR_SENSOR_ERR("get curr%u average failed, ret: %d\n", curr_index+1, ret);
        return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SYSFS_DEV_ERROR);
    }
    return ret;
}
...
static struct switch_attribute curr_sensor_debug_attr = __ATTR(debug, S_IRUGO | S_IWUSR, curr_sensor_debug_show, curr_sensor_debug_store);
static struct switch_attribute curr_sensor_loglevel_attr = __ATTR(loglevel, S_IRUGO | S_IWUSR, curr_sensor_loglevel_show, curr_sensor_loglevel_store);

static struct attribute *curr_sensor_dir_attrs[] = {
    &num_curr_att.attr,
    &curr_sensor_debug_attr.attr,
    &curr_sensor_loglevel_attr.attr,
    NULL,
};
...
static struct switch_attribute curr_crit_attr = __ATTR(crit,  S_IRUGO | S_IWUSR, curr_sensor_crit_show, curr_sensor_crit_store);
static struct switch_attribute curr_average_attr = __ATTR(average, S_IRUGO, curr_sensor_average_show, NULL);
...
static int curr_sensor_sub_single_create_kobj_and_attrs(struct kobject *parent, unsigned int index)
{
    ...
    curr_sensor = &g_curr_sensor.curr[index];
    memset(name, 0, sizeof(name));
    snprintf(name, sizeof(name), "curr%u", index+1);
...
static int curr_sensor_sub_create_kobj_and_attrs(struct kobject *parent, int curr_num)
{
    ...
    for (curr_index = 0; curr_index < curr_num; curr_index++) {
...
static void curr_sensor_sub_remove(void)
{
    ...
    if (g_curr_sensor.curr) {
        for (curr_index = 0; curr_index < g_curr_sensor.curr_number; curr_index++) {
