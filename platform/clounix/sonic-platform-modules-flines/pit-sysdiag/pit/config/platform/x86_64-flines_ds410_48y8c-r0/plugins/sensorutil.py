...
class SensorUtil(object):
    ...
    def get_all(self):
        cmd = " ".join([CMD_SHOW_PLATFORM, "temperature"])
        status,output = run_command(cmd)
        if "Thermal Not detected" in output:
            return None
        print("show platform temperature output:\n {}".format(output))

        cmd = 'cat /sys/s3ip/temp_sensor/temp5/temp_input'
        status,output = run_command(cmd)
        print("FAN_0x48 output: {}".format(output))
        cmd = 'cat /sys/s3ip/temp_sensor/temp6/temp_input'
        status,output = run_command(cmd)
        print("FAN_0x49 output: {}".format(output))

        cmd = 'redis-cli -p 6382 -n 6 hget "TEMPERATURE_INFO|TMP75C / temp 0x48" "temperature"'
        status,output = run_command(cmd)
        print("Redis FAN_0x48 output: {}".format(output))
        cmd = 'redis-cli -p 6382 -n 6 hget "TEMPERATURE_INFO|TMP75C / temp 0x49" "temperature"'
        status,output = run_command(cmd)
