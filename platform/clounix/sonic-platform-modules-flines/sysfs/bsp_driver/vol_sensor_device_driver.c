...
/*************************************main board voltage***************************************/
static ssize_t clx_get_main_board_vol_loglevel(char *buf, size_t count)
{
    PRINT_LOGLEVEL(g_dev_loglevel[CLX_DRIVER_TYPES_VOL], buf, count);
}

static ssize_t clx_set_main_board_vol_loglevel(const char *buf, size_t count)
{
    int loglevel = 0;
    ...
    g_dev_loglevel[CLX_DRIVER_TYPES_VOL] = loglevel;
    return count;
}

static ssize_t clx_get_main_board_vol_debug(char *buf, size_t count)
{
    return sprintf(buf, "read volt sensor info: \n""cat /sys/s3ip/volt_sensor/vol*/*\n");
}

static ssize_t clx_set_main_board_vol_debug(const char *buf, size_t count)
{
    return -ENOSYS;
}
...
static struct s3ip_sysfs_vol_sensor_drivers_s drivers = {
     ...
     */
    .get_loglevel = clx_get_main_board_vol_loglevel,
    .set_loglevel = clx_set_main_board_vol_loglevel,
    .get_debug = clx_get_main_board_vol_debug,
    .set_debug = clx_set_main_board_vol_debug,  
