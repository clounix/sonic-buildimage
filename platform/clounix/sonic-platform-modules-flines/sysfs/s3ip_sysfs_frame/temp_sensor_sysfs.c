...
static struct switch_attribute num_temp_attr = __ATTR(number, S_IRUGO, temp_sensor_number_show, NULL);
...
static struct switch_attribute temp_max_hyst_attr = __ATTR(max_hyst, S_IRUGO, temp_sensor_max_hyst_show, NULL);
...
static int temp_sensor_sub_single_create_kobj_and_attrs(struct kobject *parent, unsigned int index)
{
    ...
    snprintf(name, sizeof(name), "temp%u", index+1);
