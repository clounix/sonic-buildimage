...
class FwMgrUtil(object):
    ...
    def fpga_upgrade(self, fw_path):
        ...
        #update
        cmd = self.clx_update_tool + fw_path + " -b 3"
        try:
            status, retstr = run_command(cmd)
            if status != 0:
                return ret
    ...
    def get_power_cycle_cmd(self):
        find_cmd = "find /sys/devices/ -name power_cycle"
        status, retstr = run_command(find_cmd)
        if status != 0:
            print("Cannot find power_cycle node")
            return status,None
        else:
            cmdline = " echo 1 > " + retstr
            print(cmdline)
    ...
    def firmware_refresh(self, item, fw_path, fw_extra=None):
        ...
        if item == "CPLD-1" or item == "CPLD-2":
            return True
        if item == "FPGA":
            status,power_cycle_cmd = self.get_power_cycle_cmd()
