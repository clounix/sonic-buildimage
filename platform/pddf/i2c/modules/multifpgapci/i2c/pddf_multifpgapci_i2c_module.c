			...
			const char *buf, size_t count)
{
	...
	if (error != 0) {
		pddf_dbg(MULTIFPGA, "Error converting string: %d\n",
			 error);
		return -EINVAL;
	}
	...
	if (index < 0 || index >= I2C_PCI_MAX_BUS) {
		pddf_dbg(MULTIFPGA,
			 "[%s] I2C Adapter %d out of range [0, %d)\n",
	...
	pddf_dbg(MULTIFPGA, "[%s] Registered bus id: %s\n",
			...
			const char *buf, size_t count)
{
	...
	if (!i2c_privdata->i2c_adapter_registered[index]) {
		pddf_dbg(MULTIFPGA,
			 "%s: I2C Adapter %d doesn't exist\n",
			 __FUNCTION__, index);
		return -ENODEV;
	}

	pddf_dbg(MULTIFPGA,
		 "[%s] Attempting delete of bus index: %d\n",
		 __FUNCTION__, index);
...
int pddf_multifpgapci_i2c_module_init(struct pci_dev *pci_dev,
				      struct kobject *kobj)
{
	pddf_dbg(MULTIFPGA, "[%s] pci_dev %s\n", __FUNCTION__,
		 pci_name(pci_dev));
	struct pddf_multifpgapci_drvdata *pci_drvdata =
		pci_get_drvdata(pci_dev);
	struct i2c_adapter_drvdata *i2c_privdata =
	...
	if (err) {
		pddf_dbg(MULTIFPGA,
			"[%s] sysfs_create_group error, status: %d\n",
			 __FUNCTION__, err);
		return err;
	}
