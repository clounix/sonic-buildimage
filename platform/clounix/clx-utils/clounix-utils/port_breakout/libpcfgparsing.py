...
KEY_CLX_PORT_INFO_V_INDEX = 'port_index'
KEY_CLX_PORT_INFO_V_ACTIVE = 'active'
...
CPI_PORT_LIST = ["257", "258"]

LANE_NUM_PER_MACRO = 8
MAX_SPEED_PER_LANE = 50000
...
def parse_port_config_ini(port_config_file):
    ...
    for line in config.split('\n'):
        ...
        if 'speed' in title:
            ...
            max_speed = str(MAX_SPEED_PER_LANE * len(logic_lane_list))
...
def _parse_port_map(config_file):
    ...
    pat_str = r'''^init\s+set\s+port-map\s+
            ...
            max-speed=(?P<max_speed>\d+)g\s+   #max-speed
            active=(?P<active>.*)
            '''
    pat = re.compile(pat_str, re.M | re.I | re.X)
    result = pat.finditer(config_content)
    data_dict={
        KEY_CLX_PORT_INFO_V_INDEX: dict(),
        KEY_CLX_PORT_2_LOGIC_LANE: dict(),
        KEY_LOGIC_LANE_2_CLX_PORT: dict(),
        KEY_CLX_PORT_INFO_V_MAX_SPEED: dict(),
        KEY_CLX_PORT_INFO_V_ACTIVE: dict(),
    ...
    for pre in result:
        ...
        max_speed = pre.group('max_speed')

        active = pre.group('active')

        logic_lane = macro_lane_2_logic_lane(eth_macro, lane)

        clx_port_tuple = (unit, port)

        data_dict[KEY_CLX_PORT_INFO_V_INDEX][logic_lane] = port

        data_dict[KEY_CLX_PORT_INFO_V_MAX_SPEED][logic_lane] = max_speed

        data_dict[KEY_CLX_PORT_INFO_V_ACTIVE][logic_lane] = active
...
def _parse_port_property_from_file(config_file):
    ...
    pat_str = r'''^port\s+set\s+property\s+     #port set property
            (unit=(?P<unit>\d+)\s+){0,1}   #unit=0 
            portlist=(?P<portlist>[\d\-\d,]+)\s+  #portlist=0
            (?P<property>medium-type|an-lt|admin|speed|fec)=(?P<data>[\w]+)
            .*?$
            '''
    pat = re.compile(pat_str, re.M | re.I | re.X)
    result = pat.finditer(config_content)
    for pre in result:
        ...
        for p in parse_portlist_range(portlist):
            ...
            elif property_data == 'an-lt':
                attr_value = data_dict[KEY_AN]
            elif property_data == 'admin':
                attr_value = data_dict[KEY_ADMIN]
            elif property_data == 'fec':
                attr_value = data_dict[KEY_FEC]
            elif property_data == 'speed':
                attr_value = data_dict[KEY_SPEED]
            # CPI ports
            if p in CPI_PORT_LIST:               
                if KEY_CPI_PORT_INFO not in attr_value:
                    attr_value[KEY_CPI_PORT_INFO] = list()
...
def logic_lane_2_macro_lane(logic_lane):
    ...
    return (str(logic_lane // LANE_NUM_PER_MACRO), str(logic_lane % LANE_NUM_PER_MACRO))


def macro_lane_2_logic_lane(macro, clx_lane):
    return str(int(macro)*LANE_NUM_PER_MACRO + int(clx_lane))