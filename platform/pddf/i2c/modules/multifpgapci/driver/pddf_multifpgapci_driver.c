...
{
	if (!pci_dev) {
		pddf_dbg(MULTIFPGA, "%s pci_dev is NULL\n",
			 __FUNCTION__);
		return -ENODEV;
	}
...
static int pddf_pci_add_fpga(char *bdf, struct pci_dev *dev)
{
	...
	if (!fpga_data->kobj) {
		pddf_dbg(MULTIFPGA, "%s create fpga kobj failed\n",
			 __FUNCTION__);
		ret = -ENOMEM;
		goto free_fpga_data;
	}
	...
	if (!pci_drvdata->i2c_kobj) {
		pddf_dbg(MULTIFPGA, "[%s] create i2c kobj failed\n",
			 __FUNCTION__);
		ret = -ENOMEM;
		goto free_fpga_kobj;
	}
	...
	if (ret) {
		pddf_dbg(MULTIFPGA,
			 "%s create fpga sysfs attributes failed\n",
			 __FUNCTION__);
		return ret;
	}
		      ...
		      const char *buf, size_t count)
{
	...
	if (strncmp(buf, "fpgapci_init", strlen("fpgapci_init")) == 0) {
		...
		if (!fpga_node) {
			pddf_dbg(MULTIFPGA,
				 "[%s] no matching fpga data node\n",
				 __FUNCTION__);
			return -ENODEV;
		}
		if (cdata->i2c_name[0] == 0) {
			pddf_dbg(MULTIFPGA,
				 "[%s] no i2c_name specified\n",
				 __FUNCTION__);
			return -EINVAL;
		}

		pddf_dbg(MULTIFPGA, "Initializing %s as %s\n", cdata->i2c_name, bdf);
		...
		if (map_bars(bdf, pci_privdata, pci_dev)) {
			pddf_dbg(MULTIFPGA, "error_map_bars\n");
...
static int pddf_pci_config_data(struct pci_dev *dev)
{
	...
	pddf_dbg(MULTIFPGA, "[%s] PCI Config Data\n", __FUNCTION__);
	...
	if (pci_read_config_byte(dev, PCI_INTERRUPT_LINE, &irqLine)) {
		pddf_dbg(MULTIFPGA, "\tPCI_INTERRUPT_LINE Error\n");
	}
	...
	pddf_dbg(MULTIFPGA,
		"\trevisionId=0x%x, irq_line=0x%x, irq_support=%s\n",
		    ...
		    struct pci_dev *dev)
{
	...
	if (FPGAPCI_BAR_INDEX != -1) {
		...
		pddf_dbg(MULTIFPGA, "[%s] Failed to find BAR\n",
			 __FUNCTION__);
		return -1;
	}
				   ...
				   const struct pci_device_id *id)
{
	...
	pddf_dbg(MULTIFPGA, "[%s] Probed FPGA with bdf: %s\n",
		 __FUNCTION__, bdf);
			       ...
			       struct kobject *kobj)
{
	...
	if (ret != 0) {
		pddf_dbg(MULTIFPGA, "%s: Failed to register driver\n",
			 __FUNCTION__);
		return -EINVAL;
	}
...
static void pddf_multifpgapci_remove(struct pci_dev *dev)
{
	...
	if (pci_privdata == 0) {
		pddf_dbg(MULTIFPGA, "[%s]: pci_privdata is 0\n",
			 __FUNCTION__);
		return;
	}
