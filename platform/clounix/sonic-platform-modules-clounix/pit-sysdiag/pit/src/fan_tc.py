...
class FANTC(TestCaseCommon):
    ...
    def get_fan_count(self):
        try:
            return int(self.read_sysfs("/sys/s3ip/fan/num"),10)
        except Exception as e:
            self.logger.log_err(str(e), True)
            return 0
    ...
    def get_motor_count(self,fanid):
        try:
            return int(self.read_sysfs('/sys/s3ip/fan/fan{n}/num_motors'.format(n=fanid)),10)
        except Exception as e:
            self.logger.log_err(str(e), True)
            return 0
    ...
    def get_motor_property(self,fanid,motorid,name):
        try:
            v = self.read_sysfs('/sys/s3ip/fan/fan{}/motor{}/{}'.format(fanid,motorid,name))
            return int(v)
        except Exception as e:
    ...
    '''
        ...
        for n in range(self.fan_count):
            model_name = self.read_sysfs('/sys/s3ip/fan/fan{n}/model_name'.format(n=n+1))
            if not model_name :
                self.logger.log_err("FAIL to read fan {n} model_name".format(n=n+1), True)
                return E.EFAN6000

            part_number = self.read_sysfs('/sys/s3ip/fan/fan{n}/part_number'.format(n=n+1))
            if not part_number :
                self.logger.log_err("FAIL to read fan {n} part_number".format(n=n+1), True)
                return E.EFAN6000

            serial_number = self.read_sysfs('/sys/s3ip/fan/fan{n}/serial_number'.format(n=n+1))
            if not serial_number :
                self.logger.log_err("FAIL to read fan {n} serial_number".format(n=n+1), True)
                return E.EFAN6000

            if model_name == part_number or part_number == serial_number or model_name == serial_number:
                self.logger.log_err("FAIL fan {n} fru has same value".format(n=n+1), True)
                #bypass
                #return E.EFAN6000

            presence = self.read_sysfs('/sys/s3ip/fan/fan{n}/status'.format(n=n+1))
            if not presence :
                self.logger.log_err("FAIL to read fan {n} presence".format(n=n+1), True)
                return E.EFAN6000
    ...
    def test_fan_status(self):
        ...
        self.fan_count = self.get_fan_count()
        if self.fan_count != self.fan_info_dict['count']:
            self.fail_reason.append("get_fan_count fail.")
            self.logger.log_err("FAIL!", True)
            return E.EFAN6000

        for n in range(self.fan_count):
            motor_number = self.get_motor_count(n+1)
            for i in range(motor_number-1):
    ...
    '''
        ...
        for n in range(self.fan_count):
            for ratio in self.fan_info_dict['ratio_target']:
                ...
                if rt != E.OK:
                    ret = rt
                    self.fail_reason.append("fan {} control test fail.".format(n+1))
