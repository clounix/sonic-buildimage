...
static int clx_psu_probe(struct i2c_client *client)
{
    ...
    memcpy(data, &clx_psu_list[0], sizeof(struct pmbus_driver_info));

    ret = pmbus_do_probe(client, &clx_psu_list[i2c_match_id(clx_psu_id, client)->driver_data]);
    if (ret != 0) {
        goto err_pmbus;
    }
