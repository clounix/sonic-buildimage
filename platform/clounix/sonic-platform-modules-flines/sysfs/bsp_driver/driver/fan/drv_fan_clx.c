...
{
    struct fan_driver_clx *dev = (struct fan_driver_clx *)fan;
    u8 data1_l = 0, data2_l = 0, data1_h = 0, data2_h = 0;
    u16 value1 = 0, value2 = 0;
    u8 reg1 = 0, reg2 = 0;

    if (fan_index < 6)
    {
        reg1 = FAN1_INNER_RPM_OFFSET + (fan_index * 2);
        reg2 = FAN1_OUTER_RPM_OFFSET + (fan_index * 2);
    }
    else
    {
        return sprintf(buf, "0");
    }
    clx_i2c_read(dev->fan_if.bus, dev->fan_if.addr, reg1, &data1_l, 1);
    clx_i2c_read(dev->fan_if.bus, dev->fan_if.addr, (reg1 + 1), &data1_h, 1);
    value1 = ((u16)data1_h << 8) + data1_l;

    clx_i2c_read(dev->fan_if.bus, dev->fan_if.addr, reg2, &data2_l, 1);
    clx_i2c_read(dev->fan_if.bus, dev->fan_if.addr, (reg2 + 1), &data2_h, 1);
    value2 = ((u16)data2_h << 8) + data2_l;
                                              ...
                                              char *buf, size_t count)
{
    ...
    return sprintf(buf, "%d\n", (data * dev->fan_if.fan_max_speed) / 100);
}
