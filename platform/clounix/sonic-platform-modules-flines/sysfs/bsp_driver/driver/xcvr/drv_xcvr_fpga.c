...
static ssize_t platformc_xcvr_get_transceiver_power_on_status(struct clounix_priv_data *sfp, char *buf, size_t count)
{
    uint32_t val = 0;

    if (clounix_fpga_base) 
    {
        uint32_t data = readl(clounix_fpga_base + FPGA_PORT_POWER_CFG);
...
static ssize_t drv_xcvr_get_transceiver_power_on_status(void *xcvr, char *buf, size_t count)
{
    ...
    case XCVR_PLATFORM_TYPEC:
        return platformc_xcvr_get_transceiver_power_on_status(sfp, buf, count);
...
static int drv_xcvr_set_transceiver_power_on_status(void *xcvr, int status)
{
    ...
    if (clounix_fpga_base) 
    {
        ...
        writel(data, clounix_fpga_base + FPGA_PORT_POWER_CFG);
    }
    else
    {
        return -EINVAL;
