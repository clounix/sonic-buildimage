...
sensor_path = "/sys/s3ip/temp_sensor/"
num_node = "number"
dir_name = 'temp{}/'
check_list = {
    'out_put': 'value',
...
def process_adm1166_fault():
    ...
    if os.path.exists(ADM1166_1_FAULT_HISTORY_FILE):
        ...
        if adm1166_1_fault_position_history[0:2] != adm1166_1_fault_position[0:2]:
            os.system("echo " + adm1166_1_fault_position +
                      " > " + ADM1166_1_FAULT_HISTORY_FILE)
            fault_1 = 1
    ...
    if os.path.exists(ADM1166_2_FAULT_HISTORY_FILE):
        ...
        if adm1166_2_fault_position_history[0:2] != adm1166_2_fault_position[0:2]:
            os.system("echo " + adm1166_2_fault_position +
                      " > " + ADM1166_2_FAULT_HISTORY_FILE)
            fault_2 = 1
    else:
        os.system("echo " + adm1166_2_fault_position +
                  " > " + ADM1166_2_FAULT_HISTORY_FILE)
...
def cause_reboot(pos):
    fd = os.popen("cat " + sensor_path + dir_name.format(pos) + "type")
    where = fd.read()
...
def main():
    ...
    while 1:
        for pos in range(total_num):
            fd = os.popen("cat " + sensor_path +
                          dir_name.format(pos+1) + check_list['out_put'])
            curr = fd.read()
            curr = int(curr)

            fd = os.popen("cat " + sensor_path +
                          dir_name.format(pos+1) + check_list['warn'])
            warn = fd.read()
            warn = int(warn)

            fd = os.popen("cat " + sensor_path +
                          dir_name.format(pos+1) + check_list['danger'])
            danger = fd.read()
            danger = int(danger)
