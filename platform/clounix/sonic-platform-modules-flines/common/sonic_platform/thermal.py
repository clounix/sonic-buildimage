...
class Thermal(PddfThermal):
        ...
        if self.is_psu_thermal:
            ...
            if self.is_core_thermal:
                ...
                if output['mode'] != 0:
                    ...
                    output['mode'], output['status'] = getstatusoutput(cmd)
                #CPU_Package use 62 for thermal control (add:20260520)
                if 'Package' in self.thermal_obj_name:
                    output['status'] = '62000'
    ...
    def get_low_threshold(self):
        ...
        if not self.is_psu_thermal:
            if self.is_core_thermal or self.is_fpga_pvt_thermal:
                output = None
            else:
        ...
        if self.is_psu_thermal:
            ...
            if self.is_core_thermal:
                ...
                if output['mode'] != 0:
                    ...
                    output['mode'], output['status'] = getstatusoutput(cmd)
                #CPU_Package use 69 for thermal control (add:20260520)
                if 'Package' in self.thermal_obj_name:
                    output['status'] = '69000'
