...
static int adm1275_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (cindex >= 0) {
		info->m[PSC_CURRENT_OUT] = coefficients[cindex].m * 300/1000; //clounix add
		info->b[PSC_CURRENT_OUT] = coefficients[cindex].b;
		info->R[PSC_CURRENT_OUT] = coefficients[cindex].R;
	}
	if (pindex >= 0) {
		info->m[PSC_POWER] = coefficients[pindex].m * 300/1000; //clounix add
