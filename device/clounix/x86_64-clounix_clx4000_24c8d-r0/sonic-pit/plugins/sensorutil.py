...
class SensorUtil(object):
    ...
    def format_sensor_value(self,value):
        ...
        for key,values in value.items():                        
            ...
            vol_factor = self.dcdc_sensor_factor[key][0] if (self.dcdc_sensor_factor and key in self.dcdc_sensor_factor) else 1
            sensor_dcdc_voltage["Curr"] = int(values[0]) * vol_factor
            sensor_dcdc_voltage["Voltage"] = self.dcdc_info_dict[key]["low_voltage"] if key in self.dcdc_info_dict else int(values[1])
            sensor_dcdc_voltage["Power"] = self.dcdc_info_dict[key]["high_voltage"] if key in self.dcdc_info_dict else int(values[2])
