...
#define HAL_LT_DAWN_PKT_TCH_REG(__unit__, __channel__) \
    (_hal_lt_dawn_pkt_intr_vec[1 + (__channel__)].intr_reg)
#define HAL_LT_DAWN_PKT_RCH_REG(__unit__, __channel__) \
    (_hal_lt_dawn_pkt_intr_vec[5 + (__channel__)].intr_reg)

#define HAL_LT_DAWN_PKT_ERR_EVENT(__unit__) (&_hal_lt_dawn_pkt_intr_vec[0].intr_event)
#define HAL_LT_DAWN_PKT_TCH_EVENT(__unit__, __channel__) \
    (&_hal_lt_dawn_pkt_intr_vec[1 + (__channel__)].intr_event)
#define HAL_LT_DAWN_PKT_RCH_EVENT(__unit__, __channel__) \
    (&_hal_lt_dawn_pkt_intr_vec[5 + (__channel__)].intr_event)

#define HAL_LT_DAWN_PKT_ERR_CNT(__unit__) (_hal_lt_dawn_pkt_intr_vec[0].intr_cnt)
#define HAL_LT_DAWN_PKT_TCH_CNT(__unit__, __channel__) \
    (_hal_lt_dawn_pkt_intr_vec[1 + (__channel__)].intr_cnt)
#define HAL_LT_DAWN_PKT_RCH_CNT(__unit__, __channel__) \
    (_hal_lt_dawn_pkt_intr_vec[5 + (__channel__)].intr_cnt)
...
typedef struct HAL_LT_DAWN_PKT_PROFILE_NODE_S {
    HAL_LT_DAWN_PKT_NETIF_PROFILE_T *ptr_profile;
    struct HAL_LT_DAWN_PKT_PROFILE_NODE_S *ptr_next_node;

} HAL_LT_DAWN_PKT_PROFILE_NODE_T;

typedef struct {
    HAL_LT_DAWN_PKT_NETIF_INTF_T meta;
    struct net_device *ptr_net_dev;
    HAL_LT_DAWN_PKT_PROFILE_NODE_T *ptr_profile_list; /* the profiles binding to this interface */

} HAL_LT_DAWN_PKT_NETIF_PORT_DB_T;
...
static HAL_LT_DAWN_PKT_INTR_VEC_T _hal_lt_dawn_pkt_intr_vec[] = {
    ...
    {/* 4: TX_CH3   */ 1UL << 31, 0x0, 0}, {/* 5: RX_CH0   */ 1UL << 12, 0x0, 0},
    {/* 6: RX_CH1   */ 1UL << 13, 0x0, 0}, {/* 7: RX_CH2   */ 1UL << 14, 0x0, 0},
...
static HAL_LT_DAWN_PKT_NETIF_PROFILE_T
    *_ptr_hal_lt_dawn_pkt_profile_entry[HAL_LT_DAWN_PKT_NET_PROFILE_NUM_MAX] = {0};
static HAL_LT_DAWN_PKT_NETIF_PORT_DB_T _hal_lt_dawn_pkt_port_db[HAL_LT_DAWN_PKT_MAX_PORT_NUM];
 ...
 */
/*---------------------------------------------------------------------------*/
#define HAL_LT_DAWN_PKT_GET_DRV_CB_PTR(unit) (&_hal_lt_dawn_pkt_drv_cb[unit])
/*---------------------------------------------------------------------------*/
...
#define HAL_LT_DAWN_PKT_GET_TX_GPD_PTR(unit, channel, gpd) \
    (&_hal_lt_dawn_pkt_tx_cb[unit].pdma[channel].ptr_gpd_align_start_addr[gpd])
/*---------------------------------------------------------------------------*/
...
#define HAL_LT_DAWN_PKT_GET_RX_GPD_PTR(unit, channel, gpd) \
    (&_hal_lt_dawn_pkt_rx_cb[unit].pdma[channel].ptr_gpd_align_start_addr[gpd])
/*---------------------------------------------------------------------------*/
#define HAL_LT_DAWN_PKT_GET_PORT_DB(port) (&_hal_lt_dawn_pkt_port_db[port])
#define HAL_LT_DAWN_PKT_GET_PORT_PROFILE_LIST(port) \
    (_hal_lt_dawn_pkt_port_db[port].ptr_profile_list)
#define HAL_LT_DAWN_PKT_GET_PORT_NETDEV(port) _hal_lt_dawn_pkt_port_db[port].ptr_net_dev
...
#define HAL_LT_DAWN_PKT_INIT_DRV      (1UL << 0)
#define HAL_LT_DAWN_PKT_INIT_TASK     (1UL << 1)
#define HAL_LT_DAWN_PKT_INIT_INTR     (1UL << 2)
#define HAL_LT_DAWN_PKT_INIT_RX_START (1UL << 3)
    /* a bitmap to record the init status */
    UI32_T init_flag;

} HAL_LT_DAWN_PKT_DRV_CB_T;
...
typedef struct {
    HAL_LT_DAWN_PKT_TX_WAIT_T wait_mode;
...
typedef struct {
    ...
    HAL_LT_DAWN_PKT_RX_SCHED_T sched_mode;
 ...
 */
/*---------------------------------------------------------------------------*/
static HAL_LT_DAWN_PKT_DRV_CB_T _hal_lt_dawn_pkt_drv_cb[OSAL_MDC_MAX_CHIPS_PER_SYSTEM];
static HAL_LT_DAWN_PKT_TX_CB_T _hal_lt_dawn_pkt_tx_cb[OSAL_MDC_MAX_CHIPS_PER_SYSTEM];
static HAL_LT_DAWN_PKT_RX_CB_T _hal_lt_dawn_pkt_rx_cb[OSAL_MDC_MAX_CHIPS_PER_SYSTEM];
/*---------------------------------------------------------------------------*/
...
_hal_lt_dawn_pkt_enableIntr(const UI32_T unit, const UI32_T intr_bitmap)
{
    HAL_LT_DAWN_PKT_DRV_CB_T *ptr_cb = HAL_LT_DAWN_PKT_GET_DRV_CB_PTR(unit);
    CLX_IRQ_FLAGS_T irq_flag = 0;
    UI32_T intr_en = 0;

    osal_takeIsrLock(&ptr_cb->intr_lock, &irq_flag);
    osal_mdc_readPciReg(unit, HAL_LT_DAWN_PKT_GET_MMIO(HAL_LT_DAWN_PKT_CP_COMMON_INT_EN_HI),
                        &intr_en, sizeof(intr_en));
    intr_en |= intr_bitmap;
    osal_mdc_writePciReg(unit, HAL_LT_DAWN_PKT_GET_MMIO(HAL_LT_DAWN_PKT_CP_COMMON_INT_EN_HI),
                         &intr_en, sizeof(intr_en));
...
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_disableIntr(const UI32_T unit, const UI32_T intr_bitmap)
{
    HAL_LT_DAWN_PKT_DRV_CB_T *ptr_cb = HAL_LT_DAWN_PKT_GET_DRV_CB_PTR(unit);
    CLX_IRQ_FLAGS_T irq_flag = 0;
    UI32_T intr_en = 0;
...
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_maskIntr(const UI32_T unit, const UI32_T intr_bitmap)
{
    HAL_LT_DAWN_PKT_DRV_CB_T *ptr_cb = HAL_LT_DAWN_PKT_GET_DRV_CB_PTR(unit);
    CLX_IRQ_FLAGS_T irq_flag = 0;

    osal_takeIsrLock(&ptr_cb->intr_lock, &irq_flag);
    osal_mdc_writePciReg(unit, HAL_LT_DAWN_PKT_GET_MMIO(HAL_LT_DAWN_PKT_CP_COMMON_INT_MASK_CLR_HI),
...
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_unmaskIntr(const UI32_T unit, const UI32_T intr_bitmap)
{
    HAL_LT_DAWN_PKT_DRV_CB_T *ptr_cb = HAL_LT_DAWN_PKT_GET_DRV_CB_PTR(unit);
    CLX_IRQ_FLAGS_T irq_flag = 0;

    osal_takeIsrLock(&ptr_cb->intr_lock, &irq_flag);
    osal_mdc_writePciReg(unit, HAL_LT_DAWN_PKT_GET_MMIO(HAL_LT_DAWN_PKT_CP_COMMON_INT_MASK_SET_HI),
                         &intr_bitmap, sizeof(intr_bitmap));
    osal_giveIsrLock(&ptr_cb->intr_lock, &irq_flag);
...
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_dispatcher(void *ptr_cookie)
{
    ...
    osal_mdc_readPciReg(unit, HAL_LT_DAWN_PKT_GET_MMIO(HAL_LT_DAWN_PKT_CP_COMMON_INT_STAT_HI),
                        &intr_status, sizeof(UI32_T));
    intr_status = intr_status & intr_mask;
    osal_mdc_writePciReg(unit, HAL_LT_DAWN_PKT_GET_MMIO(HAL_LT_DAWN_PKT_CP_COMMON_INT_CLR_HI),
                         &intr_status, sizeof(UI32_T));
    osal_giveIsrLock(&ptr_cb->intr_lock, &irq_flag);
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_startTxChannelReg(const UI32_T unit,
                                   ...
                                   const UI32_T gpd_num)
{
    ...
    osal_mdc_writePciReg(unit,
                         HAL_LT_DAWN_PKT_GET_PDMA_TCH_REG(
                             HAL_LT_DAWN_PKT_GET_MMIO(HAL_LT_DAWN_PKT_PDMA_TCH_CMD), channel),
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_startRxChannelReg(const UI32_T unit,
                                   ...
                                   const UI32_T gpd_num)
{
    ...
    osal_mdc_writePciReg(unit,
                         HAL_LT_DAWN_PKT_GET_PDMA_RCH_REG(
                             HAL_LT_DAWN_PKT_GET_MMIO(HAL_LT_DAWN_PKT_PDMA_RCH_CMD), channel),
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_resumeTxChannelReg(const UI32_T unit,
                                    const HAL_LT_DAWN_PKT_TX_CHANNEL_T channel,
                                    const UI32_T gpd_num)
{
    HAL_LT_DAWN_PKT_TCH_CMD_REG_T tch_cmd;
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_resumeRxChannelReg(const UI32_T unit,
                                    const HAL_LT_DAWN_PKT_RX_CHANNEL_T channel,
                                    const UI32_T gpd_num)
{
    HAL_LT_DAWN_PKT_RCH_CMD_REG_T rch_cmd;
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_stopTxChannelReg(const UI32_T unit, const HAL_LT_DAWN_PKT_TX_CHANNEL_T channel)
{
    HAL_LT_DAWN_PKT_TCH_CMD_REG_T tch_cmd;
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_stopRxChannelReg(const UI32_T unit, const HAL_LT_DAWN_PKT_RX_CHANNEL_T channel)
{
    HAL_LT_DAWN_PKT_RCH_CMD_REG_T rch_cmd;
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_setTxGpdStartAddrReg(const UI32_T unit,
                                      const HAL_LT_DAWN_PKT_TX_CHANNEL_T channel,
                                      ...
                                      const UI32_T gpd_ring_sz)
{
    ...
    rc = osal_mdc_writePciReg(
        unit,
        HAL_LT_DAWN_PKT_GET_PDMA_TCH_REG(
            HAL_LT_DAWN_PKT_GET_MMIO(HAL_LT_DAWN_PKT_PDMA_TCH_GPD_RING_START_ADDR_LO), channel),
    ...
    if (CLX_E_OK == rc) {
        ...
        rc = osal_mdc_writePciReg(
            unit,
            HAL_LT_DAWN_PKT_GET_PDMA_TCH_REG(
                HAL_LT_DAWN_PKT_GET_MMIO(HAL_LT_DAWN_PKT_PDMA_TCH_GPD_RING_START_ADDR_HI), channel),
    ...
    if (CLX_E_OK == rc) {
        ...
        rc = osal_mdc_writePciReg(
            unit,
            HAL_LT_DAWN_PKT_GET_PDMA_TCH_REG(
                HAL_LT_DAWN_PKT_GET_MMIO(HAL_LT_DAWN_PKT_PDMA_TCH_GPD_RING_SIZE), channel),
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_setRxGpdStartAddrReg(const UI32_T unit,
                                      const HAL_LT_DAWN_PKT_RX_CHANNEL_T channel,
                                      const CLX_ADDR_T gpd_start_addr,
                                      const UI32_T gpd_ring_sz)
{
    ...
    rc = osal_mdc_writePciReg(
        unit,
        HAL_LT_DAWN_PKT_GET_PDMA_RCH_REG(
            HAL_LT_DAWN_PKT_GET_MMIO(HAL_LT_DAWN_PKT_PDMA_RCH_GPD_RING_START_ADDR_LO), channel),
    ...
    if (CLX_E_OK == rc) {
        ...
        rc = osal_mdc_writePciReg(
            unit,
            HAL_LT_DAWN_PKT_GET_PDMA_RCH_REG(
                HAL_LT_DAWN_PKT_GET_MMIO(HAL_LT_DAWN_PKT_PDMA_RCH_GPD_RING_START_ADDR_HI), channel),
    ...
    if (CLX_E_OK == rc) {
        ...
        rc = osal_mdc_writePciReg(
            unit,
            HAL_LT_DAWN_PKT_GET_PDMA_RCH_REG(
                HAL_LT_DAWN_PKT_GET_MMIO(HAL_LT_DAWN_PKT_PDMA_RCH_GPD_RING_SIZE), channel),
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_maskAllTxL2IsrReg(const UI32_T unit, const HAL_LT_DAWN_PKT_TX_CHANNEL_T channel)
{
    UI32_T reg = 0;
    ...
    osal_mdc_writePciReg(unit,
                         HAL_LT_DAWN_PKT_GET_PDMA_TCH_REG(
                             HAL_LT_DAWN_PKT_GET_MMIO(HAL_LT_DAWN_PKT_PDMA_TCH_INT_MASK), channel),
                         &reg, sizeof(UI32_T));
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_maskAllRxL2IsrReg(const UI32_T unit, const HAL_LT_DAWN_PKT_RX_CHANNEL_T channel)
{
    UI32_T reg = 0;
    ...
    osal_mdc_writePciReg(unit,
                         HAL_LT_DAWN_PKT_GET_PDMA_RCH_REG(
                             HAL_LT_DAWN_PKT_GET_MMIO(HAL_LT_DAWN_PKT_PDMA_RCH_INT_MASK), channel),
                         &reg, sizeof(UI32_T));
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_unmaskAllTxL2IsrReg(const UI32_T unit, const HAL_LT_DAWN_PKT_TX_CHANNEL_T channel)
{
    UI32_T reg = 0;
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_unmaskAllRxL2IsrReg(const UI32_T unit, const HAL_LT_DAWN_PKT_RX_CHANNEL_T channel)
{
    UI32_T reg = 0;
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_clearTxL2IsrStatusReg(const UI32_T unit,
                                       const HAL_LT_DAWN_PKT_TX_CHANNEL_T channel,
                                       const HAL_LT_DAWN_PKT_TX_CHANNEL_L2_ISR_T isr_bitmap)
{
    ...
    osal_mdc_writePciReg(unit,
                         HAL_LT_DAWN_PKT_GET_PDMA_TCH_REG(
                             HAL_LT_DAWN_PKT_GET_MMIO(HAL_LT_DAWN_PKT_PDMA_TCH_INT_CLR), channel),
                         &reg, sizeof(UI32_T));
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_clearRxL2IsrStatusReg(const UI32_T unit,
                                       const HAL_LT_DAWN_PKT_RX_CHANNEL_T channel,
                                       const HAL_LT_DAWN_PKT_RX_CHANNEL_L2_ISR_T isr_bitmap)
{
    ...
    osal_mdc_writePciReg(unit,
                         HAL_LT_DAWN_PKT_GET_PDMA_RCH_REG(
                             HAL_LT_DAWN_PKT_GET_MMIO(HAL_LT_DAWN_PKT_PDMA_RCH_INT_CLR), channel),
                         &reg, sizeof(UI32_T));
 ...
 */
CLX_ERROR_NO_T
hal_lt_dawn_pkt_getTxIntrCnt(const UI32_T unit, const UI32_T channel, UI32_T *ptr_intr_cnt)
{
 ...
 */
CLX_ERROR_NO_T
hal_lt_dawn_pkt_getRxIntrCnt(const UI32_T unit, const UI32_T channel, UI32_T *ptr_intr_cnt)
{
 ...
 */
CLX_ERROR_NO_T
hal_lt_dawn_pkt_getTxKnlCnt(const UI32_T unit, void *ptr_data)
{
    ...
    osal_io_copyToUser(&ptr_cookie->tx_cnt, &ptr_tx_cb->cnt, sizeof(HAL_LT_DAWN_PKT_TX_CNT_T));
    return (CLX_E_OK);
}
 ...
 */
CLX_ERROR_NO_T
hal_lt_dawn_pkt_getRxKnlCnt(const UI32_T unit, void *ptr_data)
{
    HAL_LT_DAWN_PKT_RX_CB_T *ptr_rx_cb = HAL_LT_DAWN_PKT_GET_RX_CB_PTR(unit);
    HAL_LT_DAWN_PKT_IOCTL_CH_CNT_COOKIE_T *ptr_cookie = ptr_data;

    osal_io_copyToUser(&ptr_cookie->rx_cnt, &ptr_rx_cb->cnt, sizeof(HAL_LT_DAWN_PKT_RX_CNT_T));
    return (CLX_E_OK);
}
 ...
 */
CLX_ERROR_NO_T
hal_lt_dawn_pkt_clearTxKnlCnt(const UI32_T unit, void *ptr_data)
{
    HAL_LT_DAWN_PKT_TX_CB_T *ptr_tx_cb = HAL_LT_DAWN_PKT_GET_TX_CB_PTR(unit);
 ...
 */
CLX_ERROR_NO_T
hal_lt_dawn_pkt_clearRxKnlCnt(const UI32_T unit, void *ptr_data)
{
    HAL_LT_DAWN_PKT_RX_CB_T *ptr_rx_cb = HAL_LT_DAWN_PKT_GET_RX_CB_PTR(unit);
 ...
 */
CLX_ERROR_NO_T
hal_lt_dawn_pkt_setPortAttr(const UI32_T unit, void *ptr_data)
{
#define HAL_LT_DAWN_PKT_PORT_STATUS_UP   (1)
#define HAL_LT_DAWN_PKT_PORT_STATUS_DOWN (0)
    struct net_device *ptr_net_dev;
    struct net_device_priv *ptr_priv;
    UI32_T port;
    UI32_T status;
    UI32_T speed;
    HAL_LT_DAWN_PKT_IOCTL_PORT_COOKIE_T *ptr_cookie = ptr_data;
    ...
    /* coverity: port used as an index of _hal_lt_dawn_pkt_port_db[129] */
    if (HAL_LT_DAWN_PKT_MAX_PORT_NUM > port) {
    ...
    if ((NULL != ptr_net_dev) && (port < HAL_LT_DAWN_PKT_MAX_PORT_NUM)) {
        if (HAL_LT_DAWN_PKT_PORT_STATUS_UP == status) {
...
static void
_hal_lt_dawn_pkt_lockRxChannelAll(const UI32_T unit)
{
    UI32_T rch;
...
static void
_hal_lt_dawn_pkt_unlockRxChannelAll(const UI32_T unit)
{
    UI32_T rch;
    HAL_LT_DAWN_PKT_RX_PDMA_T *ptr_rx_pdma;
...
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_setIntfProperty(const UI32_T unit, void *ptr_data)
{
    UI32_T intf_id;
    ...
    UI32_T param1;
    CLX_ERROR_NO_T rc;
    HAL_LT_DAWN_PKT_NL_IOCTL_COOKIE_T *ptr_cookie = ptr_data;
...
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_getIntfProperty(const UI32_T unit, void *ptr_data)
{
    UI32_T intf_id;
    NETIF_NL_INTF_PROPERTY_T property;
    UI32_T param0;
...
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_createNetlink(const UI32_T unit, void *ptr_data)
{
...
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_destroyNetlink(const UI32_T unit, void *ptr_data)
{
    UI32_T netlink_id;
    CLX_ERROR_NO_T rc;
    HAL_LT_DAWN_PKT_NL_IOCTL_COOKIE_T *ptr_cookie = ptr_data;
    ...
    if (HAL_LT_DAWN_PKT_NET_PROFILE_NUM_MAX > netlink_id) {
...
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_getNetlink(const UI32_T unit, void *ptr_data)
{
    UI32_T id;
    NETIF_NL_NETLINK_T netlink;
    CLX_ERROR_NO_T rc;
    ...
    /* coverity: intf_id used as an offset of _netif_nl_cb.fam_entry[256] */
    if (HAL_LT_DAWN_PKT_NET_PROFILE_NUM_MAX > id) {
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_enQueue(HAL_LT_DAWN_PKT_SW_QUEUE_T *ptr_que, void *ptr_data)
{
    CLX_ERROR_NO_T rc = CLX_E_OK;
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_deQueue(HAL_LT_DAWN_PKT_SW_QUEUE_T *ptr_que, void **pptr_data)
{
    CLX_ERROR_NO_T rc = CLX_E_OK;
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_getQueueCount(HAL_LT_DAWN_PKT_SW_QUEUE_T *ptr_que, UI32_T *ptr_count)
{
    CLX_ERROR_NO_T rc = CLX_E_OK;
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_allocRxPayloadBuf(const UI32_T unit, const UI32_T channel, const UI32_T gpd_idx)
{
    CLX_ERROR_NO_T rc = CLX_E_NO_MEMORY;
    HAL_LT_DAWN_PKT_RX_CB_T *ptr_rx_cb = HAL_LT_DAWN_PKT_GET_RX_CB_PTR(unit);
    volatile HAL_LT_DAWN_PKT_RX_GPD_T *ptr_rx_gpd =
        HAL_LT_DAWN_PKT_GET_RX_GPD_PTR(unit, channel, gpd_idx);
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_freeRxPayloadBuf(const UI32_T unit, const UI32_T channel, const UI32_T gpd_idx)
{
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_freeRxPayloadBufGpd(const UI32_T unit, HAL_LT_DAWN_PKT_RX_SW_GPD_T *ptr_sw_gpd)
{
    CLX_ERROR_NO_T rc = CLX_E_OTHERS;
    CLX_ADDR_T phy_addr = 0;
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_initTxPdmaRing(const UI32_T unit, const HAL_LT_DAWN_PKT_TX_CHANNEL_T channel)
{
    CLX_ERROR_NO_T rc = CLX_E_OK;
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_initRxPdmaRing(const UI32_T unit, const HAL_LT_DAWN_PKT_RX_CHANNEL_T channel)
{
    CLX_ERROR_NO_T rc = CLX_E_OK;
    HAL_LT_DAWN_PKT_RX_PDMA_T *ptr_rx_pdma = HAL_LT_DAWN_PKT_GET_RX_PDMA_PTR(unit, channel);
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_initRxPdmaRingBuf(const UI32_T unit, const HAL_LT_DAWN_PKT_RX_CHANNEL_T channel)
{
    CLX_ERROR_NO_T rc = CLX_E_OK;
    HAL_LT_DAWN_PKT_RX_CB_T *ptr_rx_cb = HAL_LT_DAWN_PKT_GET_RX_CB_PTR(unit);
    HAL_LT_DAWN_PKT_RX_PDMA_T *ptr_rx_pdma = HAL_LT_DAWN_PKT_GET_RX_PDMA_PTR(unit, channel);
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_deinitRxPdmaRingBuf(const UI32_T unit, const HAL_LT_DAWN_PKT_RX_CHANNEL_T channel)
{
    CLX_ERROR_NO_T rc = CLX_E_OK;
    HAL_LT_DAWN_PKT_RX_PDMA_T *ptr_rx_pdma = HAL_LT_DAWN_PKT_GET_RX_PDMA_PTR(unit, channel);
    volatile HAL_LT_DAWN_PKT_RX_GPD_T *ptr_rx_gpd = NULL;
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_recoverTxPdma(const UI32_T unit, const HAL_LT_DAWN_PKT_TX_CHANNEL_T channel)
{
    CLX_ERROR_NO_T rc = CLX_E_OK;
    HAL_LT_DAWN_PKT_TX_PDMA_T *ptr_tx_pdma = HAL_LT_DAWN_PKT_GET_TX_PDMA_PTR(unit, channel);
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_recoverRxPdma(const UI32_T unit, const HAL_LT_DAWN_PKT_RX_CHANNEL_T channel)
{
    CLX_ERROR_NO_T rc = CLX_E_OK;
    HAL_LT_DAWN_PKT_RX_PDMA_T *ptr_rx_pdma = HAL_LT_DAWN_PKT_GET_RX_PDMA_PTR(unit, channel);
 ...
 */
static void
_hal_lt_dawn_pkt_freeTxGpdList(UI32_T unit, HAL_LT_DAWN_PKT_TX_SW_GPD_T *ptr_sw_gpd)
{
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_freeRxGpdList(UI32_T unit,
 ...
 */
static void
_hal_lt_dawn_pkt_txEnQueueBulk(const UI32_T unit, const UI32_T channel, const UI32_T number)
{
    HAL_LT_DAWN_PKT_TX_PDMA_T *ptr_tx_pdma = HAL_LT_DAWN_PKT_GET_TX_PDMA_PTR(unit, channel);
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_strictTxDeQueue(const UI32_T unit, void *ptr_data)
{
    CLX_ERROR_NO_T rc = CLX_E_OK;
    HAL_LT_DAWN_PKT_TX_CB_T *ptr_tx_cb = HAL_LT_DAWN_PKT_GET_TX_CB_PTR(unit);
    HAL_LT_DAWN_PKT_TX_SW_GPD_T *ptr_sw_gpd = NULL;
 ...
 */
static void
_hal_lt_dawn_pkt_rxCheckReason(volatile HAL_LT_DAWN_PKT_RX_GPD_T *ptr_rx_gpd,
                               HAL_LT_DAWN_PKT_NETIF_PROFILE_T *ptr_profile,
                               BOOL_T *ptr_hit_prof)
{
    ...
    if (0 == (ptr_profile->flags & HAL_LT_DAWN_PKT_NETIF_PROFILE_FLAGS_REASON)) {
...
static BOOL_T
_hal_lt_dawn_pkt_comparePatternWithPayload(volatile HAL_LT_DAWN_PKT_RX_GPD_T *ptr_rx_gpd,
...
static void
_hal_lt_dawn_pkt_rxCheckPattern(volatile HAL_LT_DAWN_PKT_RX_GPD_T *ptr_rx_gpd,
                                HAL_LT_DAWN_PKT_NETIF_PROFILE_T *ptr_profile,
                                BOOL_T *ptr_hit_prof)
{
    UI32_T idx;
    ...
    if ((ptr_profile->flags &
         (HAL_LT_DAWN_PKT_NETIF_PROFILE_FLAGS_PATTERN_0 |
          HAL_LT_DAWN_PKT_NETIF_PROFILE_FLAGS_PATTERN_1 |
          HAL_LT_DAWN_PKT_NETIF_PROFILE_FLAGS_PATTERN_2 |
          HAL_LT_DAWN_PKT_NETIF_PROFILE_FLAGS_PATTERN_3)) != 0) {
...
static void
_hal_lt_dawn_pkt_matchUserProfile(volatile HAL_LT_DAWN_PKT_RX_GPD_T *ptr_rx_gpd,
                                  HAL_LT_DAWN_PKT_PROFILE_NODE_T *ptr_profile_list,
                                  HAL_LT_DAWN_PKT_NETIF_PROFILE_T **pptr_profile_hit)
{
    HAL_LT_DAWN_PKT_PROFILE_NODE_T *ptr_curr_node = ptr_profile_list;
    BOOL_T hit;
...
static void
_hal_lt_dawn_pkt_getPacketDest(volatile HAL_LT_DAWN_PKT_RX_GPD_T *ptr_rx_gpd,
                               HAL_LT_DAWN_PKT_DEST_T *ptr_dest,
                               void **pptr_cookie)
{
    UI32_T port;
    HAL_LT_DAWN_PKT_PROFILE_NODE_T *ptr_profile_list;
    HAL_LT_DAWN_PKT_NETIF_PROFILE_T *ptr_profile_hit;

    port = ptr_rx_gpd->itmh_eth.igr_phy_port;
    ptr_profile_list = HAL_LT_DAWN_PKT_GET_PORT_PROFILE_LIST(port);

    _hal_lt_dawn_pkt_matchUserProfile(ptr_rx_gpd, ptr_profile_list, &ptr_profile_hit);
    if (NULL != ptr_profile_hit) {
        if (HAL_LT_DAWN_PKT_NETIF_RX_DST_NETLINK == ptr_profile_hit->dst_type) {
 ...
 */
static void
_hal_lt_dawn_pkt_rxEnQueue(const UI32_T unit,
                           ...
                           HAL_LT_DAWN_PKT_RX_SW_GPD_T *ptr_sw_gpd)
{
    ...
    if ((HAL_LT_DAWN_PKT_DEST_NETDEV == dest_type) || (HAL_LT_DAWN_PKT_DEST_NETLINK == dest_type)) {
...
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_flushRxQueue(const UI32_T unit, HAL_LT_DAWN_PKT_SW_QUEUE_T *ptr_que)
{
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_schedRxDeQueue(const UI32_T unit, void *ptr_data)
{
    HAL_LT_DAWN_PKT_IOCTL_RX_COOKIE_T ioctl_data;
    ...
    UI32_T buf_len = 0;
    CLX_ERROR_NO_T rc = CLX_E_OK;
    HAL_LT_DAWN_PKT_IOCTL_RX_COOKIE_T *ptr_cookie = ptr_data;
    ...
    if ((que_cnt > 0) && (queue < HAL_LT_DAWN_PKT_RX_QUEUE_NUM)) {
        ...
        if (CLX_E_OK == rc) {
            ...
            osal_io_copyFromUser(&ioctl_data, ptr_cookie,
                                 sizeof(HAL_LT_DAWN_PKT_IOCTL_RX_COOKIE_T));
            ...
            while (NULL != ptr_sw_gpd_knl) {
                ...
                osal_io_copyToUser((void *)((CLX_HUGE_T)ioctl_gpd.hw_gpd_addr),
                                   &ptr_sw_gpd_knl->rx_gpd, sizeof(HAL_LT_DAWN_PKT_RX_GPD_T));
                /* copy buf */
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_waitTxDone(const UI32_T unit,
                            const HAL_LT_DAWN_PKT_TX_CHANNEL_T channel,
                            HAL_LT_DAWN_PKT_TX_SW_GPD_T *ptr_sw_gpd)
{
    ...
    if (HAL_LT_DAWN_PKT_TX_WAIT_ASYNC == ptr_tx_cb->wait_mode) {
        ;
    } else if (HAL_LT_DAWN_PKT_TX_WAIT_SYNC_INTR == ptr_tx_cb->wait_mode) {
    ...
    } else if (HAL_LT_DAWN_PKT_TX_WAIT_SYNC_POLL == ptr_tx_cb->wait_mode) {
...
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_resumeAllIntf(const UI32_T unit)
{
    struct net_device *ptr_net_dev = NULL;
    UI32_T port;

    /* Unregister net devices by id */
    for (port = 0; port < HAL_LT_DAWN_PKT_MAX_PORT_NUM; port++) {
        ptr_net_dev = HAL_LT_DAWN_PKT_GET_PORT_NETDEV(port);
        if (NULL != ptr_net_dev) {
...
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_suspendAllIntf(const UI32_T unit)
{
    struct net_device *ptr_net_dev = NULL;
    UI32_T port;
...
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_stopAllIntf(const UI32_T unit)
{
    struct net_device *ptr_net_dev = NULL;
    UI32_T port;
 ...
 */
CLX_ERROR_NO_T
hal_lt_dawn_pkt_sendGpd(const UI32_T unit,
                        const HAL_LT_DAWN_PKT_TX_CHANNEL_T channel,
                        HAL_LT_DAWN_PKT_TX_SW_GPD_T *ptr_sw_gpd)
{
    CLX_ERROR_NO_T rc = CLX_E_OK;
    ...
    if (0 != (ptr_cb->init_flag & HAL_LT_DAWN_PKT_INIT_TASK)) {
...
#define HAL_LT_DAWN_PKT_KNL_TX_RING_AVBL_GPD_LOW (HAL_LT_DAWN_PORT_NUM)
                if (ptr_tx_pdma->free_gpd_num < HAL_LT_DAWN_PKT_KNL_TX_RING_AVBL_GPD_LOW) {
                    ...
                    _hal_lt_dawn_pkt_suspendAllIntf(unit);
                }
            } else {
                rc = CLX_E_TABLE_FULL;
            }
...
/* ----------------------------------------------------------------------------------- Rx Init */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_rxStop(const UI32_T unit)
{
    CLX_ERROR_NO_T rc = CLX_E_OK;
    ...
    if (0 == (ptr_cb->init_flag & HAL_LT_DAWN_PKT_INIT_RX_START)) {
    ...
    if ((0 == (ptr_cb->init_flag & HAL_LT_DAWN_PKT_INIT_TASK)) ||
        (0 == (ptr_cb->init_flag & HAL_LT_DAWN_PKT_INIT_DRV))) {
    ...
    ptr_cb->init_flag &= (~HAL_LT_DAWN_PKT_INIT_RX_START);
...
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_rxStart(const UI32_T unit)
{
    CLX_ERROR_NO_T rc = CLX_E_OK;
    HAL_LT_DAWN_PKT_RX_CHANNEL_T channel = 0;
    HAL_LT_DAWN_PKT_RX_CB_T *ptr_rx_cb = HAL_LT_DAWN_PKT_GET_RX_CB_PTR(unit);
 ...
 */
CLX_ERROR_NO_T
hal_lt_dawn_pkt_setRxKnlConfig(const UI32_T unit, void *ptr_data)
{
    CLX_ERROR_NO_T rc = CLX_E_OK;
    HAL_LT_DAWN_PKT_RX_CB_T *ptr_rx_cb = HAL_LT_DAWN_PKT_GET_RX_CB_PTR(unit);
    HAL_LT_DAWN_PKT_DRV_CB_T *ptr_cb = HAL_LT_DAWN_PKT_GET_DRV_CB_PTR(unit);
    HAL_LT_DAWN_PKT_IOCTL_RX_TYPE_T rx_type = HAL_LT_DAWN_PKT_IOCTL_RX_TYPE_LAST;
    HAL_LT_DAWN_PKT_IOCTL_RX_COOKIE_T *ptr_cookie = ptr_data;

    osal_io_copyFromUser(&rx_type, &ptr_cookie->rx_type, sizeof(HAL_LT_DAWN_PKT_IOCTL_RX_TYPE_T));

    if (HAL_LT_DAWN_PKT_IOCTL_RX_TYPE_DEINIT == rx_type) {
        _hal_lt_dawn_pkt_rxStop(unit);
    }
    if (HAL_LT_DAWN_PKT_IOCTL_RX_TYPE_INIT == rx_type) {
 ...
 */
CLX_ERROR_NO_T
hal_lt_dawn_pkt_getRxKnlConfig(const UI32_T unit, void *ptr_data)
{
    HAL_LT_DAWN_PKT_RX_CB_T *ptr_rx_cb = HAL_LT_DAWN_PKT_GET_RX_CB_PTR(unit);
    HAL_LT_DAWN_PKT_IOCTL_RX_COOKIE_T *ptr_cookie = ptr_data;
 ...
 */
CLX_ERROR_NO_T
hal_lt_dawn_pkt_deinitTask(const UI32_T unit, void *ptr_data)
{
    HAL_LT_DAWN_PKT_DRV_CB_T *ptr_cb = HAL_LT_DAWN_PKT_GET_DRV_CB_PTR(unit);
    HAL_LT_DAWN_PKT_TX_CB_T *ptr_tx_cb = HAL_LT_DAWN_PKT_GET_TX_CB_PTR(unit);
    HAL_LT_DAWN_PKT_RX_CB_T *ptr_rx_cb = HAL_LT_DAWN_PKT_GET_RX_CB_PTR(unit);
    ...
    _hal_lt_dawn_pkt_stopAllIntf(unit);
    ...
    /* Set the flag to record init state */
    ptr_cb->init_flag &= (~HAL_LT_DAWN_PKT_INIT_TASK);
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_deinitTxPdma(const UI32_T unit, const HAL_LT_DAWN_PKT_TX_CHANNEL_T channel)
{
    HAL_LT_DAWN_PKT_TX_CB_T *ptr_tx_cb = HAL_LT_DAWN_PKT_GET_TX_CB_PTR(unit);
    HAL_LT_DAWN_PKT_TX_PDMA_T *ptr_tx_pdma = HAL_LT_DAWN_PKT_GET_TX_PDMA_PTR(unit, channel);
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_deinitRxPdma(const UI32_T unit, const HAL_LT_DAWN_PKT_RX_CHANNEL_T channel)
{
    HAL_LT_DAWN_PKT_RX_PDMA_T *ptr_rx_pdma = HAL_LT_DAWN_PKT_GET_RX_PDMA_PTR(unit, channel);
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_deinitPktCb(const UI32_T unit)
{
    HAL_LT_DAWN_PKT_DRV_CB_T *ptr_cb = HAL_LT_DAWN_PKT_GET_DRV_CB_PTR(unit);
    UI32_T idx = 0, vec = sizeof(_hal_lt_dawn_pkt_intr_vec) / sizeof(HAL_LT_DAWN_PKT_INTR_VEC_T);
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_deinitPktTxCb(const UI32_T unit)
{
    CLX_ERROR_NO_T rc = CLX_E_OK;
    HAL_LT_DAWN_PKT_TX_CB_T *ptr_tx_cb = HAL_LT_DAWN_PKT_GET_TX_CB_PTR(unit);
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_deinitPktRxCb(const UI32_T unit)
{
    CLX_ERROR_NO_T rc = CLX_E_OK;
    HAL_LT_DAWN_PKT_RX_CB_T *ptr_rx_cb = HAL_LT_DAWN_PKT_GET_RX_CB_PTR(unit);
    HAL_LT_DAWN_PKT_RX_CHANNEL_T channel = 0;
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_deinitL1Isr(const UI32_T unit)
{
    UI32_T idx = 0, vec = sizeof(_hal_lt_dawn_pkt_intr_vec) / sizeof(HAL_LT_DAWN_PKT_INTR_VEC_T);
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_deinitL2Isr(const UI32_T unit)
{
    ...
    osal_mdc_writePciReg(unit, HAL_LT_DAWN_PKT_GET_MMIO(HAL_LT_DAWN_PKT_PDMA_ERR_INT_MASK_SET),
                         &isr_status, sizeof(UI32_T));

    isr_status = 0x0;
    osal_mdc_writePciReg(unit, HAL_LT_DAWN_PKT_GET_MMIO(HAL_LT_DAWN_PKT_PDMA_ERR_INT_EN),
                         &isr_status, sizeof(UI32_T));
 ...
 */
CLX_ERROR_NO_T
hal_lt_dawn_pkt_deinitPktDrv(const UI32_T unit, void *ptr_data)
{
    HAL_LT_DAWN_PKT_DRV_CB_T *ptr_cb = HAL_LT_DAWN_PKT_GET_DRV_CB_PTR(unit);
    CLX_ERROR_NO_T rc = CLX_E_OK;

    if (0 == (ptr_cb->init_flag & HAL_LT_DAWN_PKT_INIT_DRV)) {
    ...
    ptr_cb->init_flag &= (~HAL_LT_DAWN_PKT_INIT_DRV);
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_handleTxErrStat(const UI32_T unit, const HAL_LT_DAWN_PKT_TX_CHANNEL_T channel)
{
    HAL_LT_DAWN_PKT_TX_CB_T *ptr_tx_cb = HAL_LT_DAWN_PKT_GET_TX_CB_PTR(unit);
    HAL_LT_DAWN_PKT_TX_PDMA_T *ptr_tx_pdma = HAL_LT_DAWN_PKT_GET_TX_PDMA_PTR(unit, channel);
    CLX_IRQ_FLAGS_T irg_flags;

    if (HAL_LT_DAWN_PKT_TX_WAIT_SYNC_INTR == ptr_tx_cb->wait_mode) {
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_handleRxErrStat(const UI32_T unit, const HAL_LT_DAWN_PKT_RX_CHANNEL_T channel)
{
    HAL_LT_DAWN_PKT_RX_PDMA_T *ptr_rx_pdma = HAL_LT_DAWN_PKT_GET_RX_PDMA_PTR(unit, channel);
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_handleTxL2Isr(const UI32_T unit, const HAL_LT_DAWN_PKT_TX_CHANNEL_T channel)
{
    ...
    osal_mdc_readPciReg(unit,
                        HAL_LT_DAWN_PKT_GET_PDMA_TCH_REG(
                            HAL_LT_DAWN_PKT_GET_MMIO(HAL_LT_DAWN_PKT_PDMA_TCH_INT_STAT), channel),
    ...
    if (0 != (isr_status & HAL_LT_DAWN_PKT_TX_CHANNEL_L2_ISR_GPD_HWO_ERROR)) {
        OSAL_PRINT((OSAL_DBG_ERR | OSAL_DBG_TX), "u=%u, txch=%u, pdma gpd hwo err\n", unit,
                   channel);
        _hal_lt_dawn_pkt_clearTxL2IsrStatusReg(unit, channel,
                                               HAL_LT_DAWN_PKT_TX_CHANNEL_L2_ISR_GPD_HWO_ERROR);
    ...
    if (0 != (isr_status & HAL_LT_DAWN_PKT_TX_CHANNEL_L2_ISR_GPD_DMA_READ_ERROR)) {
        ...
        _hal_lt_dawn_pkt_clearTxL2IsrStatusReg(
            unit, channel, HAL_LT_DAWN_PKT_TX_CHANNEL_L2_ISR_GPD_DMA_READ_ERROR);
    ...
    if (0 != (isr_status & HAL_LT_DAWN_PKT_TX_CHANNEL_L2_ISR_LEN_MISMATCH_ERROR)) {
        OSAL_PRINT((OSAL_DBG_ERR | OSAL_DBG_TX), "u=%u, txch=%u, pdma len mismatch err\n", unit,
                   channel);
        _hal_lt_dawn_pkt_clearTxL2IsrStatusReg(
            unit, channel, HAL_LT_DAWN_PKT_TX_CHANNEL_L2_ISR_LEN_MISMATCH_ERROR);
    ...
    if (0 != (isr_status & HAL_LT_DAWN_PKT_TX_CHANNEL_L2_ISR_PKTPL_DMA_READ_ERROR)) {
        OSAL_PRINT((OSAL_DBG_ERR | OSAL_DBG_TX), "u=%u, txch=%u, pdma pkt buf dma read err\n", unit,
                   channel);
        _hal_lt_dawn_pkt_clearTxL2IsrStatusReg(
            unit, channel, HAL_LT_DAWN_PKT_TX_CHANNEL_L2_ISR_PKTPL_DMA_READ_ERROR);
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_handleRxL2Isr(const UI32_T unit, const HAL_LT_DAWN_PKT_RX_CHANNEL_T channel)
{
    ...
    osal_mdc_readPciReg(unit,
                        HAL_LT_DAWN_PKT_GET_PDMA_RCH_REG(
                            HAL_LT_DAWN_PKT_GET_MMIO(HAL_LT_DAWN_PKT_PDMA_RCH_INT_STAT), channel),
                        &isr_status, sizeof(isr_status));
    ...
    if (0 != (isr_status & HAL_LT_DAWN_PKT_RX_CHANNEL_L2_ISR_AVAIL_GPD_LOW)) {
        ...
        _hal_lt_dawn_pkt_clearRxL2IsrStatusReg(unit, channel,
                                               HAL_LT_DAWN_PKT_RX_CHANNEL_L2_ISR_AVAIL_GPD_LOW);
...
_hal_lt_dawn_pkt_handleErrorTask(void *ptr_argv)
{
    ...
    do {
        ...
        osal_mdc_readPciReg(unit, HAL_LT_DAWN_PKT_GET_MMIO(HAL_LT_DAWN_PKT_PDMA_ERR_INT_STAT),
                            &isr_status, sizeof(UI32_T));
        ...
        if (0 != isr_status) {
            osal_mdc_writePciReg(unit, HAL_LT_DAWN_PKT_GET_MMIO(HAL_LT_DAWN_PKT_PDMA_ERR_INT_CLR),
                                 &isr_status, sizeof(UI32_T));
...
_hal_lt_dawn_pkt_handleTxDoneTask(void *ptr_argv)
{
    ...
    HAL_LT_DAWN_PKT_TX_CHANNEL_T channel =
        (HAL_LT_DAWN_PKT_TX_CHANNEL_T)((HAL_LT_DAWN_PKT_ISR_COOKIE_T *)ptr_argv)->channel;
    /* control block */
    HAL_LT_DAWN_PKT_TX_CB_T *ptr_tx_cb = HAL_LT_DAWN_PKT_GET_TX_CB_PTR(unit);
    HAL_LT_DAWN_PKT_TX_PDMA_T *ptr_tx_pdma = HAL_LT_DAWN_PKT_GET_TX_PDMA_PTR(unit, channel);
    volatile HAL_LT_DAWN_PKT_TX_GPD_T *ptr_tx_gpd = NULL;
    ...
    do {
        ...
        _hal_lt_dawn_pkt_resumeAllIntf(unit);
...
_hal_lt_dawn_pkt_handleRxDoneTask(void *ptr_argv)
{
    /* cookie or index */
    UI32_T unit = ((HAL_LT_DAWN_PKT_ISR_COOKIE_T *)ptr_argv)->unit;
    HAL_LT_DAWN_PKT_RX_CHANNEL_T channel =
        (HAL_LT_DAWN_PKT_RX_CHANNEL_T)((HAL_LT_DAWN_PKT_ISR_COOKIE_T *)ptr_argv)->channel;
    ...
    do {
        ...
        while (loop_cnt > 0) {
            ...
            if (TRUE == first) {
                ptr_sw_first_gpd =
                    (HAL_LT_DAWN_PKT_RX_SW_GPD_T *)osal_alloc(sizeof(HAL_LT_DAWN_PKT_RX_SW_GPD_T));
                ptr_sw_gpd = ptr_sw_first_gpd;
                if (NULL != ptr_sw_gpd) {
                    ...
                    OSAL_PRINT((OSAL_DBG_RX | OSAL_DBG_ERR),
                               ...
                               sizeof(HAL_LT_DAWN_PKT_RX_SW_GPD_T));
                    break;
                }
            } else {
                ptr_sw_gpd->ptr_next =
            ...
            while (CLX_E_OK !=
                   _hal_lt_dawn_pkt_allocRxPayloadBuf(unit, channel, ptr_rx_pdma->cur_idx)) {
                ptr_rx_cb->cnt.no_memory++;
...
static void
_hal_lt_dawn_pkt_net_dev_tx_callback(const UI32_T unit,
                                     HAL_LT_DAWN_PKT_TX_SW_GPD_T *ptr_sw_gpd,
 ...
 */
CLX_ERROR_NO_T
hal_lt_dawn_pkt_initTask(const UI32_T unit, void *ptr_data)
{
    CLX_ERROR_NO_T rc = CLX_E_OK;
    HAL_LT_DAWN_PKT_DRV_CB_T *ptr_cb = HAL_LT_DAWN_PKT_GET_DRV_CB_PTR(unit);
    HAL_LT_DAWN_PKT_TX_CB_T *ptr_tx_cb = HAL_LT_DAWN_PKT_GET_TX_CB_PTR(unit);
    ...
    rc = osal_createThread("ERROR", NETIF_PKT_PKT_ERROR_ISR_THREAD_STACK,
                           NETIF_PKT_PKT_ERROR_ISR_THREAD_PRI, _hal_lt_dawn_pkt_handleErrorTask,
                           (void *)((CLX_HUGE_T)unit), &ptr_cb->err_task_id);

    /* Init handleTxDoneTask */
    for (channel = 0; ((channel < HAL_LT_DAWN_PKT_TX_CHANNEL_LAST) && (CLX_E_OK == rc));
         channel++) {
        ...
        rc = osal_createThread("TX_ISR", NETIF_PKT_PKT_TX_ISR_THREAD_STACK,
                               NETIF_PKT_PKT_TX_ISR_THREAD_PRI, _hal_lt_dawn_pkt_handleTxDoneTask,
                               (void *)&ptr_tx_cb->isr_task_cookie[channel],
                               &ptr_tx_cb->isr_task_id[channel]);
    }

    /* Init handleRxDoneTask */
    for (channel = 0; ((channel < HAL_LT_DAWN_PKT_RX_CHANNEL_LAST) && (CLX_E_OK == rc));
         channel++) {
        ...
        rc = osal_createThread("RX_ISR", NETIF_PKT_PKT_RX_ISR_THREAD_STACK,
                               NETIF_PKT_PKT_RX_ISR_THREAD_PRI, _hal_lt_dawn_pkt_handleRxDoneTask,
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_initTxPdma(const UI32_T unit, const HAL_LT_DAWN_PKT_TX_CHANNEL_T channel)
{
    CLX_ERROR_NO_T rc = CLX_E_OK;
    HAL_LT_DAWN_PKT_TX_CB_T *ptr_tx_cb = HAL_LT_DAWN_PKT_GET_TX_CB_PTR(unit);
    HAL_LT_DAWN_PKT_TX_PDMA_T *ptr_tx_pdma = HAL_LT_DAWN_PKT_GET_TX_PDMA_PTR(unit, channel);
    ...
    if (NULL != ptr_tx_pdma->ptr_gpd_start_addr) {
        ...
        ptr_tx_pdma->ptr_gpd_align_start_addr =
            (HAL_LT_DAWN_PKT_TX_GPD_T *)HAL_LT_DAWN_PKT_PDMA_ALIGN_ADDR(
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_initRxPdma(const UI32_T unit, const HAL_LT_DAWN_PKT_RX_CHANNEL_T channel)
{
    CLX_ERROR_NO_T rc = CLX_E_OK;
    HAL_LT_DAWN_PKT_RX_CB_T *ptr_rx_cb = HAL_LT_DAWN_PKT_GET_RX_CB_PTR(unit);
    HAL_LT_DAWN_PKT_RX_PDMA_T *ptr_rx_pdma = HAL_LT_DAWN_PKT_GET_RX_PDMA_PTR(unit, channel);
    ...
    if (NULL != ptr_rx_pdma->ptr_gpd_start_addr) {
        ...
        ptr_rx_pdma->ptr_gpd_align_start_addr =
            (HAL_LT_DAWN_PKT_RX_GPD_T *)HAL_LT_DAWN_PKT_PDMA_ALIGN_ADDR(
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_initPktCb(const UI32_T unit)
{
    HAL_LT_DAWN_PKT_DRV_CB_T *ptr_cb = HAL_LT_DAWN_PKT_GET_DRV_CB_PTR(unit);
    UI32_T idx = 0, vec = sizeof(_hal_lt_dawn_pkt_intr_vec) / sizeof(HAL_LT_DAWN_PKT_INTR_VEC_T);

    osal_memset(ptr_cb, 0x0, sizeof(HAL_LT_DAWN_PKT_DRV_CB_T));
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_initPktTxCb(const UI32_T unit)
{
    CLX_ERROR_NO_T rc = CLX_E_OK;
    HAL_LT_DAWN_PKT_TX_CB_T *ptr_tx_cb = HAL_LT_DAWN_PKT_GET_TX_CB_PTR(unit);
    HAL_LT_DAWN_PKT_TX_CHANNEL_T channel = 0;
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_initPktRxCb(const UI32_T unit)
{
    CLX_ERROR_NO_T rc = CLX_E_OK;
    HAL_LT_DAWN_PKT_RX_CB_T *ptr_rx_cb = HAL_LT_DAWN_PKT_GET_RX_CB_PTR(unit);
    HAL_LT_DAWN_PKT_RX_CHANNEL_T channel = 0;
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_initL1Isr(const UI32_T unit)
{
    UI32_T idx = 0, vec = sizeof(_hal_lt_dawn_pkt_intr_vec) / sizeof(HAL_LT_DAWN_PKT_INTR_VEC_T);
 ...
 */
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_initL2Isr(const UI32_T unit)
{
    HAL_LT_DAWN_PKT_L2_ISR_T isr_status = 0x0;
...
CLX_ERROR_NO_T
_hal_lt_dawn_pkt_resetIosCreditCfg(const UI32_T unit)
{
...
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_addProfToList(HAL_LT_DAWN_PKT_NETIF_PROFILE_T *ptr_new_profile,
                               HAL_LT_DAWN_PKT_PROFILE_NODE_T **pptr_profile_list)
{
    HAL_LT_DAWN_PKT_PROFILE_NODE_T *ptr_new_prof_node;
    HAL_LT_DAWN_PKT_PROFILE_NODE_T *ptr_curr_node, *ptr_prev_node;

    ptr_new_prof_node = osal_alloc(sizeof(HAL_LT_DAWN_PKT_PROFILE_NODE_T));
...
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_addProfToAllIntf(HAL_LT_DAWN_PKT_NETIF_PROFILE_T *ptr_new_profile)
{
    UI32_T port;
    HAL_LT_DAWN_PKT_NETIF_PORT_DB_T *ptr_port_db;

    for (port = 0; port < HAL_LT_DAWN_PKT_MAX_PORT_NUM; port++) {
        ptr_port_db = HAL_LT_DAWN_PKT_GET_PORT_DB(port);
        /* Shall we check if the interface is ever created on the port?? */
        /* if (NULL != ptr_port_db->ptr_net_dev) */
        if (1) {
            _hal_lt_dawn_pkt_addProfToList(ptr_new_profile, &ptr_port_db->ptr_profile_list);
        }
    }
...
static HAL_LT_DAWN_PKT_NETIF_PROFILE_T *
_hal_lt_dawn_pkt_delProfFromListById(const UI32_T id,
                                     HAL_LT_DAWN_PKT_PROFILE_NODE_T **pptr_profile_list)
{
    HAL_LT_DAWN_PKT_PROFILE_NODE_T *ptr_temp_node;
    HAL_LT_DAWN_PKT_PROFILE_NODE_T *ptr_curr_node, *ptr_prev_node;
    HAL_LT_DAWN_PKT_NETIF_PROFILE_T *ptr_profile = NULL;
    ;
...
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_delProfFromAllIntfById(const UI32_T id)
{
    UI32_T port;
    HAL_LT_DAWN_PKT_NETIF_PORT_DB_T *ptr_port_db;
    ...
    for (port = 0; port < HAL_LT_DAWN_PKT_MAX_PORT_NUM; port++) {
        ...
        /* if (NULL != ptr_port_db->ptr_net_dev) */
        if (1) {
            _hal_lt_dawn_pkt_delProfFromListById(id, &ptr_port_db->ptr_profile_list);
        }
    }
    return (CLX_E_OK);
}
...
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_allocProfEntry(HAL_LT_DAWN_PKT_NETIF_PROFILE_T *ptr_profile)
{
    UI32_T idx;

    for (idx = 0; idx < HAL_LT_DAWN_PKT_NET_PROFILE_NUM_MAX; idx++) {
        if (NULL == _ptr_hal_lt_dawn_pkt_profile_entry[idx]) {
            OSAL_PRINT(OSAL_DBG_PROFILE, "alloc prof entry failed, id=%d\n", idx);
            _ptr_hal_lt_dawn_pkt_profile_entry[idx] = ptr_profile;
            ptr_profile->id = idx;
            return (CLX_E_OK);
        }
    }
...
static HAL_LT_DAWN_PKT_NETIF_PROFILE_T *
_hal_lt_dawn_pkt_freeProfEntry(const UI32_T id)
{
    HAL_LT_DAWN_PKT_NETIF_PROFILE_T *ptr_profile = NULL;

    if (id < HAL_LT_DAWN_PKT_NET_PROFILE_NUM_MAX) {
        ptr_profile = _ptr_hal_lt_dawn_pkt_profile_entry[id];
        _ptr_hal_lt_dawn_pkt_profile_entry[id] = NULL;
    }
...
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_destroyAllIntf(const UI32_T unit)
{
    HAL_LT_DAWN_PKT_NETIF_PORT_DB_T *ptr_port_db;
    UI32_T port = 0;
    ...
    for (port = 0; port < HAL_LT_DAWN_PKT_MAX_PORT_NUM; port++) {
        ...
        if (NULL != ptr_port_db->ptr_net_dev) /* valid intf */
        {
             ...
             */
            /* _hal_lt_dawn_pkt_destroyProfList(ptr_port_db->ptr_profile_list); */

            osal_memset(ptr_port_db, 0x0, sizeof(HAL_LT_DAWN_PKT_NETIF_PORT_DB_T));
        }
    }
...
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_delProfListOnAllIntf(const UI32_T unit)
{
    HAL_LT_DAWN_PKT_NETIF_PORT_DB_T *ptr_port_db;
    UI32_T port = 0;
    HAL_LT_DAWN_PKT_PROFILE_NODE_T *ptr_curr_node, *ptr_next_node;
...
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_destroyAllProfile(const UI32_T unit)
{
    HAL_LT_DAWN_PKT_NETIF_PROFILE_T *ptr_profile;
    UI32_T prof_id;

    _hal_lt_dawn_pkt_delProfListOnAllIntf(unit);

    for (prof_id = 0; prof_id < CLX_NETIF_PROFILE_NUM_MAX; prof_id++) {
        ptr_profile = _hal_lt_dawn_pkt_freeProfEntry(prof_id);
        if (NULL != ptr_profile) {
            OSAL_PRINT(OSAL_DBG_PROFILE,
 ...
 */
CLX_ERROR_NO_T
hal_lt_dawn_pkt_initPktDrv(const UI32_T unit, void *ptr_data)
{
    CLX_ERROR_NO_T rc = CLX_E_OK;
    UI32_T channel = 0;
    ...
    if (0 != (ptr_cb->init_flag & HAL_LT_DAWN_PKT_INIT_DRV)) {
        ...
        hal_lt_dawn_pkt_deinitTask(unit, NULL);

        OSAL_PRINT(OSAL_DBG_ERR, "u=%u, deinit pkt drv\n", unit);
        hal_lt_dawn_pkt_deinitPktDrv(unit, NULL);

        OSAL_PRINT(OSAL_DBG_ERR, "u=%u, destroy all prof\n", unit);
        _hal_lt_dawn_pkt_destroyAllProfile(unit);
        ...
        _hal_lt_dawn_pkt_destroyAllIntf(unit);
    }
...
CLX_ERROR_NO_T
hal_lt_dawn_pkt_prepareGpd(const UI32_T unit,
                           ...
                           HAL_LT_DAWN_PKT_TX_SW_GPD_T *ptr_sw_gpd)
{
    ...
    ptr_sw_gpd->tx_gpd.crcc = HAL_LT_DAWN_PKT_CRCC_SUM_BY_HW;
 ...
 * net_dev_ops */
static int
_hal_lt_dawn_pkt_net_dev_init(struct net_device *ptr_net_dev)
{
    return 0;
}

static int
_hal_lt_dawn_pkt_net_dev_open(struct net_device *ptr_net_dev)
{
...
static int
_hal_lt_dawn_pkt_net_dev_stop(struct net_device *ptr_net_dev)
{
    netif_stop_queue(ptr_net_dev);
    return 0;
}
...
static int
_hal_lt_dawn_pkt_net_dev_ioctl(struct net_device *ptr_net_dev, struct ifreq *ptr_ifreq, int cmd)
{
    return 0;
}

static netdev_tx_t
_hal_lt_dawn_pkt_net_dev_tx(struct sk_buff *ptr_skb, struct net_device *ptr_net_dev)
{
    ...
    if (NULL == ptr_sw_gpd) {
        ...
        if (0x0 == phy_addr) {
            ...
            /* prepare gpd */
            hal_lt_dawn_pkt_prepareGpd(unit, phy_addr, ptr_skb->len, ptr_priv->port, ptr_sw_gpd);
...
static void
_hal_lt_dawn_pkt_net_dev_tx_timeout(struct net_device *ptr_net_dev, unsigned int txqueue)
#else
static void
_hal_lt_dawn_pkt_net_dev_tx_timeout(struct net_device *ptr_net_dev)
#endif
{
    netif_stop_queue(ptr_net_dev);
...
_hal_lt_dawn_pkt_net_dev_get_stats(struct net_device *ptr_net_dev)
{
    struct net_device_priv *ptr_priv = netdev_priv(ptr_net_dev);
...
static int
_hal_lt_dawn_pkt_net_dev_set_mtu(struct net_device *ptr_net_dev, int new_mtu)
{
...
static int
_hal_lt_dawn_pkt_net_dev_set_mac(struct net_device *ptr_net_dev, void *ptr_mac_addr)
{
...
static void
_hal_lt_dawn_pkt_net_dev_set_rx_mode(struct net_device *ptr_dev)
{
...
static struct net_device_ops _hal_lt_dawn_pkt_net_dev_ops = {
    .ndo_init = _hal_lt_dawn_pkt_net_dev_init,
    .ndo_open = _hal_lt_dawn_pkt_net_dev_open,
    .ndo_stop = _hal_lt_dawn_pkt_net_dev_stop,
    .ndo_do_ioctl = _hal_lt_dawn_pkt_net_dev_ioctl,
    .ndo_start_xmit = _hal_lt_dawn_pkt_net_dev_tx,
    .ndo_tx_timeout = _hal_lt_dawn_pkt_net_dev_tx_timeout,
    .ndo_get_stats = _hal_lt_dawn_pkt_net_dev_get_stats,
    .ndo_change_mtu = _hal_lt_dawn_pkt_net_dev_set_mtu,
    .ndo_set_mac_address = _hal_lt_dawn_pkt_net_dev_set_mac,
    .ndo_set_rx_mode = _hal_lt_dawn_pkt_net_dev_set_rx_mode,
};

#if LINUX_VERSION_CODE < KERNEL_VERSION(5, 10, 0)
static int
_hal_lt_dawn_pkt_net_dev_ethtool_get(struct net_device *ptr_dev, struct ethtool_cmd *ptr_cmd)
{
    struct net_device_priv *ptr_priv;
...
static struct ethtool_ops _hal_lt_dawn_pkt_net_dev_ethtool_ops = {
#if LINUX_VERSION_CODE < KERNEL_VERSION(5, 10, 0)
    .get_settings = _hal_lt_dawn_pkt_net_dev_ethtool_get,
#endif
    .get_link = ethtool_op_get_link,
};

static void
_hal_lt_dawn_pkt_setup(struct net_device *ptr_net_dev)
{
    struct net_device_priv *ptr_priv = netdev_priv(ptr_net_dev);
...
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_createIntf(const UI32_T unit, void *ptr_data)
{
    HAL_LT_DAWN_PKT_NETIF_INTF_T net_intf = {0};
    HAL_LT_DAWN_PKT_NETIF_PORT_DB_T *ptr_port_db;
    struct net_device *ptr_net_dev = NULL;
    struct net_device_priv *ptr_priv = NULL;
    CLX_ERROR_NO_T rc = CLX_E_OK;
    HAL_LT_DAWN_PKT_IOCTL_NETIF_COOKIE_T *ptr_cookie = ptr_data;
    ...
    osal_io_copyFromUser(&net_intf, &ptr_cookie->net_intf, sizeof(HAL_LT_DAWN_PKT_NETIF_INTF_T));
...
#if defined(HAL_LT_DAWN_PKT_FORCR_REMOVE_DUPLICATE_NETDEV)
    ...
    /* coverity:net_intf.port used as an index into _hal_lt_dawn_pkt_port_db[129] */
    if (HAL_LT_DAWN_PKT_MAX_PORT_NUM > net_intf.port) {
        ptr_port_db = HAL_LT_DAWN_PKT_GET_PORT_DB(net_intf.port);
    } else {
    ...
    if (ptr_port_db->ptr_net_dev == NULL) {
        ...
        ptr_net_dev = alloc_netdev(sizeof(struct net_device_priv), net_intf.name, NET_NAME_UNKNOWN,
                                   _hal_lt_dawn_pkt_setup);
#else
        ptr_net_dev =
            alloc_netdev(sizeof(struct net_device_priv), net_intf.name, _hal_lt_dawn_pkt_setup);
#endif
        ...
        osal_memcpy(&ptr_port_db->meta, &net_intf, sizeof(HAL_LT_DAWN_PKT_NETIF_INTF_T));

        ptr_port_db->ptr_net_dev = ptr_net_dev;

        /* Copy the intf-id to user space */
        osal_io_copyToUser(&ptr_cookie->net_intf, &net_intf, sizeof(HAL_LT_DAWN_PKT_NETIF_INTF_T));
    } else {
...
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_destroyIntf(const UI32_T unit, void *ptr_data)
{
    HAL_LT_DAWN_PKT_NETIF_INTF_T net_intf = {0};
    HAL_LT_DAWN_PKT_NETIF_PORT_DB_T *ptr_port_db;
    UI32_T port = 0;
...
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_traverseProfList(UI32_T intf_id, HAL_LT_DAWN_PKT_PROFILE_NODE_T *ptr_prof_list)
{
    HAL_LT_DAWN_PKT_PROFILE_NODE_T *ptr_curr_node;
...
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_getIntf(const UI32_T unit, void *ptr_data)
{
    HAL_LT_DAWN_PKT_NETIF_INTF_T net_intf = {0};
    HAL_LT_DAWN_PKT_NETIF_PORT_DB_T *ptr_port_db;
    UI32_T port = 0;
    ...
    for (port = 0; port < HAL_LT_DAWN_PKT_MAX_PORT_NUM; port++) {
        ...
        if (NULL != ptr_port_db->ptr_net_dev) /* valid intf */
        {
            if (ptr_port_db->meta.id == net_intf.id) {
                ...
                _hal_lt_dawn_pkt_traverseProfList(net_intf.id, ptr_port_db->ptr_profile_list);
                osal_io_copyToUser(&ptr_cookie->net_intf, &ptr_port_db->meta,
                                   sizeof(HAL_LT_DAWN_PKT_NETIF_INTF_T));
                rc = CLX_E_OK;
                break;
            }
        }
...
static HAL_LT_DAWN_PKT_NETIF_PROFILE_T *
_hal_lt_dawn_pkt_getProfEntry(const UI32_T id)
{
    HAL_LT_DAWN_PKT_NETIF_PROFILE_T *ptr_profile = NULL;

    if (id < HAL_LT_DAWN_PKT_NET_PROFILE_NUM_MAX) {
        if (NULL != _ptr_hal_lt_dawn_pkt_profile_entry[id]) {
            ptr_profile = _ptr_hal_lt_dawn_pkt_profile_entry[id];
        }
    }
...
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_createProfile(const UI32_T unit, void *ptr_data)
{
    HAL_LT_DAWN_PKT_NETIF_PROFILE_T *ptr_profile;
    HAL_LT_DAWN_PKT_NETIF_PORT_DB_T *ptr_port_db;
    CLX_ERROR_NO_T rc;
    ...
    ptr_profile = osal_alloc(sizeof(HAL_LT_DAWN_PKT_NETIF_PROFILE_T));
    osal_io_copyFromUser(ptr_profile, &ptr_cookie->net_profile,
                         sizeof(HAL_LT_DAWN_PKT_NETIF_PROFILE_T));
    ...
    rc = _hal_lt_dawn_pkt_allocProfEntry(ptr_profile);
    if (CLX_E_OK == rc) {
        /* Insert the profile to the corresponding (port) interface */
        if ((ptr_profile->flags & HAL_LT_DAWN_PKT_NETIF_PROFILE_FLAGS_PORT) != 0) {
            OSAL_PRINT(OSAL_DBG_PROFILE, "u=%u, bind prof to phy port=%d\n", unit,
                       ptr_profile->port);
            ptr_port_db = HAL_LT_DAWN_PKT_GET_PORT_DB(ptr_profile->port);
            _hal_lt_dawn_pkt_addProfToList(ptr_profile, &ptr_port_db->ptr_profile_list);
        } else {
            OSAL_PRINT(OSAL_DBG_PROFILE, "u=%u, bind prof to all intf\n", unit);
            _hal_lt_dawn_pkt_addProfToAllIntf(ptr_profile);
        }
...
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_destroyProfile(const UI32_T unit, void *ptr_data)
{
    HAL_LT_DAWN_PKT_NETIF_PROFILE_T profile = {0};
    HAL_LT_DAWN_PKT_NETIF_PROFILE_T *ptr_profile;
    CLX_ERROR_NO_T rc = CLX_E_OK;
    HAL_LT_DAWN_PKT_IOCTL_NETIF_COOKIE_T *ptr_cookie = ptr_data;
    ...
    _hal_lt_dawn_pkt_delProfFromAllIntfById(profile.id);

    ptr_profile = _hal_lt_dawn_pkt_freeProfEntry(profile.id);
    if (NULL != ptr_profile) {
...
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_getProfile(const UI32_T unit, void *ptr_data)
{
    HAL_LT_DAWN_PKT_NETIF_PROFILE_T profile = {0};
    HAL_LT_DAWN_PKT_NETIF_PROFILE_T *ptr_profile;
    CLX_ERROR_NO_T rc = CLX_E_OK;
    ...
    ptr_profile = _hal_lt_dawn_pkt_getProfEntry(profile.id);
    if (NULL != ptr_profile) {
        osal_io_copyToUser(&ptr_cookie->net_profile, ptr_profile,
                           sizeof(HAL_LT_DAWN_PKT_NETIF_PROFILE_T));
    } else {
...
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_getIntfCnt(const UI32_T unit, void *ptr_data)
{
    HAL_LT_DAWN_PKT_NETIF_INTF_T net_intf = {0};
    HAL_LT_DAWN_PKT_NETIF_INTF_CNT_T intf_cnt = {0};
    HAL_LT_DAWN_PKT_NETIF_PORT_DB_T *ptr_port_db;
    struct net_device_priv *ptr_priv;
    UI32_T port = 0;
    CLX_ERROR_NO_T rc = CLX_E_ENTRY_NOT_FOUND;
    ...
    osal_io_copyToUser(&ptr_cookie->cnt, &intf_cnt, sizeof(HAL_LT_DAWN_PKT_NETIF_INTF_CNT_T));
    osal_io_copyToUser(&ptr_cookie->rc, &rc, sizeof(CLX_ERROR_NO_T));
...
static CLX_ERROR_NO_T
_hal_lt_dawn_pkt_clearIntfCnt(const UI32_T unit, void *ptr_data)
{
    HAL_LT_DAWN_PKT_NETIF_INTF_T net_intf = {0};
    HAL_LT_DAWN_PKT_NETIF_PORT_DB_T *ptr_port_db;
    struct net_device_priv *ptr_priv;
 ...
 */
static void
_hal_lt_dawn_pkt_dev_tx_callback(const UI32_T unit,
                                 HAL_LT_DAWN_PKT_TX_SW_GPD_T *ptr_sw_gpd,
...
hal_lt_dawn_pkt_dev_tx(const UI32_T unit, void *ptr_data)
{
    ...
    /* copy the tx-cookie */
    osal_io_copyFromUser(&tx_cookie, ptr_data, sizeof(HAL_LT_DAWN_PKT_IOCTL_TX_COOKIE_T));
    ...
    while (1) {
        ...
        if (HAL_LT_DAWN_PKT_CH_LAST_GPD == ptr_sw_gpd_knl->tx_gpd.ch) {
            ...
            ptr_sw_gpd_knl->ptr_next =
                (HAL_LT_DAWN_PKT_TX_SW_GPD_T *)osal_alloc(sizeof(HAL_LT_DAWN_PKT_TX_SW_GPD_T));
            ptr_sw_gpd_knl = ptr_sw_gpd_knl->ptr_next;
            idx++;
        }
    }
    ...
    if (HAL_LT_DAWN_PKT_TX_CHANNEL_LAST > channel) {
 ...
 */
CLX_ERROR_NO_T
hal_lt_dawn_pkt_init(const UI32_T unit)
{
    /* Init Thread */
    osal_init();

    /* Reset all database*/
    osal_memset(_hal_lt_dawn_pkt_port_db, 0x0,
                (HAL_LT_DAWN_PKT_MAX_PORT_NUM * sizeof(HAL_LT_DAWN_PKT_NETIF_PORT_DB_T)));
    osal_memset(_hal_lt_dawn_pkt_rx_cb, 0x0,
                OSAL_MDC_MAX_CHIPS_PER_SYSTEM * sizeof(HAL_LT_DAWN_PKT_RX_CB_T));
    osal_memset(_hal_lt_dawn_pkt_tx_cb, 0x0,
                OSAL_MDC_MAX_CHIPS_PER_SYSTEM * sizeof(HAL_LT_DAWN_PKT_TX_CB_T));
    osal_memset(_hal_lt_dawn_pkt_drv_cb, 0x0,
                OSAL_MDC_MAX_CHIPS_PER_SYSTEM * sizeof(HAL_LT_DAWN_PKT_DRV_CB_T));
...
CLX_ERROR_NO_T
hal_lt_dawn_pkt_exit(const UI32_T unit)
{
...
#define HAL_LT_DAWN_PKT_MODULE_EXIT_HOLD_TIME_US (1000000)
    osal_sleepThread(HAL_LT_DAWN_PKT_MODULE_EXIT_HOLD_TIME_US);
