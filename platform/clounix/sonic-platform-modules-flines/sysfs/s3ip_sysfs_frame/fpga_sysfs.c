...
static ssize_t fpga_power_history_reg_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    unsigned int fpga_index;
    int ret;

    check_p(g_fpga_drv);
    check_p(g_fpga_drv->get_main_board_fpga_power_history_reg);

    fpga_index = obj->index;
    FPGA_DBG("fpga index: %u\n", fpga_index);
    ret = g_fpga_drv->get_main_board_fpga_power_history_reg(fpga_index, buf, PAGE_SIZE);
    if (ret < 0) {
        FPGA_ERR("get fpga%u test register failed, ret: %d\n", fpga_index, ret);
        return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SYSFS_DEV_ERROR);
    }
...
static ssize_t fpga_power_history_ctrl_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    unsigned int fpga_index;
    int ret;

    check_p(g_fpga_drv);
    check_p(g_fpga_drv->get_main_board_fpga_power_history_ctrl);

    fpga_index = obj->index;
    FPGA_DBG("fpga index: %u\n", fpga_index);
    ret = g_fpga_drv->get_main_board_fpga_power_history_ctrl(fpga_index, buf, PAGE_SIZE);
    if (ret < 0) 
    {
        FPGA_ERR("get fpga%u test register failed, ret: %d\n", fpga_index, ret);
        return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SYSFS_DEV_ERROR);
...
static ssize_t fpga_power_history_ctrl_store(struct switch_obj *obj, struct switch_attribute *attr, const char* buf, size_t count)
{
    unsigned int fpga_index, value;
    int ret;

    check_p(g_fpga_drv);
    check_p(g_fpga_drv->set_main_board_fpga_power_history_ctrl);
    ...
    if (ret) 
    {
        FPGA_ERR("fpga%u invalid input: '%s', expected 0 or 1.\n", fpga_index, buf);
        return ret;
    }

    ret = g_fpga_drv->set_main_board_fpga_power_history_ctrl(fpga_index, value);
    if (ret < 0)
    {
        FPGA_ERR("fpga%u set power history ctrl failed, value:0x%x, ret: %d.\n", fpga_index, value, ret);
        return ret;
    }

    FPGA_DBG("fpga%u set power history ctrl success, value: 0x%x.\n", fpga_index, value);
    return count;
}

/************************************fpga dir and attrs*******************************************/
static struct switch_attribute fpga_number_attr = __ATTR(number, S_IRUGO, fpga_number_show, NULL);
...
static struct switch_attribute fpga_history_reg_attr = __ATTR(power_status_history, S_IRUGO, fpga_power_history_reg_show, NULL);
static struct switch_attribute fpga_history_ctrl_attr = __ATTR(power_status_ctrl, S_IRUGO | S_IWUSR, fpga_power_history_ctrl_show, fpga_power_history_ctrl_store);
...
static struct attribute *fpga_attrs[] = {
    ...
    &fpga_history_reg_attr.attr,
    &fpga_history_ctrl_attr.attr,
    NULL,
};
