...
# clx sfp error status bit definition
CLX_SFP_STATUS_BIT_UNKNOWN_MODULE = 0x00010000
CLX_SFP_STATUS_BIT_MODULE_ERROR = 0x00020000
CLX_SFP_STATUS_BIT_MODULE_DATAPATH_ERROR = 0x00040000
...
class PddfSfp(SfpOptoeBase):
    ...
    def get_sfp_error(self):
        ...
        """
        error_code = SfpBase.SFP_STATUS_BIT_INSERTED
        error_desc_dict = SfpBase.SFP_ERROR_BIT_TO_DESCRIPTION_DICT
        error_desc = []
        if not self.get_presence():
            return SfpBase.SFP_STATUS_BIT_REMOVED, ''

        # bad eeprom
        buf = self.read_eeprom(0x00, 1)
        if buf is None or buf[0] == 00:
            error_code = SfpBase.SFP_ERROR_BIT_BLOCKING
            return error_code, error_desc_dict[error_code]

        # unknown module
        api = self.get_xcvr_api()
        if api is None:
            error_code = CLX_SFP_STATUS_BIT_UNKNOWN_MODULE
            return error_code, "Unknown module"
        ...
        if isinstance(api, CmisApi):
            ...
            if status['module_state'] != CmisCodes.MODULE_STATE[3]:
                error_code = CLX_SFP_STATUS_BIT_MODULE_ERROR
            ...
            try:
                status_flags = api.get_transceiver_status_flags()
                if status_flags['datapath_firmware_fault']:
                    return CLX_SFP_STATUS_BIT_MODULE_ERROR, "Datapath firmware fault"
                if status_flags['module_firmware_fault']:
                    return CLX_SFP_STATUS_BIT_MODULE_ERROR, "Module firmware fault"
            except Exception:
                pass

            try:
                dom_flags = api.get_transceiver_dom_flags()
                if 'tempHAlarm' in dom_flags and dom_flags['tempHAlarm']:
                    error_desc.append("High temperature")
                if 'tempLAlarm' in dom_flags and dom_flags['tempLAlarm']:
                    error_desc.append("Low temperature")
                if 'vccHAlarm' in dom_flags and dom_flags['vccHAlarm']:
                    error_desc.append("High supply voltage")
                if 'vccLAlarm' in dom_flags and dom_flags['vccLAlarm']:
                    error_desc.append("Low supply voltage")
                if 'lasertempHAlarm' in dom_flags and dom_flags['lasertempHAlarm']:
                    error_desc.append("High laser temperature")
                if 'lasertempLAlarm' in dom_flags and dom_flags['lasertempLAlarm']:
            ...
            if api.is_flat_memory():
                return SfpBase.SFP_STATUS_BIT_INSERTED, ''

            # datapath error check for cmis
            config_lanes_list = []
            for lane in range(1, api.NUM_CHANNELS + 1):
                name = "{}_{}_{}".format(consts.STAGED_CTRL_APSEL_FIELD, 0, lane)
                appl = api.xcvr_eeprom.read(name)
                if (appl is None) or ((appl >> 4) == 0):
                    continue
                config_lanes_list.append(lane)

            for lane in config_lanes_list:
                name = "config_state_hostlane{}".format(lane)
                if name in status and status[name] != CmisCodes.CONFIG_STATUS[1]:
                    error_desc.append(status[name])
                    break
                name = "DP{}State".format(lane)
                if name in status and status[name] != CmisCodes.DATAPATH_STATE[4]:
                    error_desc.append(status[name])
                    break
            if len(error_desc) > 0:
                error_code = CLX_SFP_STATUS_BIT_MODULE_DATAPATH_ERROR
                return error_code, "|".join(error_desc)

            try:
                for lane in config_lanes_list:
                    name = "tx%dOutputStatus" % lane
                    if name in status and status[name] != True:
                        error_desc.append('TX output signal muted or invalid')
                        break
                for lane in config_lanes_list:
                    name = "rx%dOutputStatusHostlane" % lane
                    if name in status and status[name] != True:
                        error_desc.append('RX output signal muted or invalid')
                        break
                for lane in config_lanes_list:
                    name = "tx%ddisable" % lane
                    if name in status and status[name] != 'N/A' and status[name] != False:
            ...
            try:
                for lane in config_lanes_list:
                    name = "tx%dfault" % lane
                    if name in status_flags and status_flags[name] != 'N/A' and status_flags[name] != False:
                        error_desc.append('TX fault')
                        break
                for lane in config_lanes_list:
                    name = "tx%dlos_hostlane" % lane
                    if name in status_flags and status_flags[name] != 'N/A' and status_flags[name] != False:
                        error_desc.append('TX loss of signal')
                        break
                for lane in config_lanes_list:
                    name = "tx%dcdrlol_hostlane" % lane
                    if name in status_flags and status_flags[name] != 'N/A' and status_flags[name] != False:
    ...
    def get_error_description(self):
        ...
        try:
            ...
            if error_code != SfpBase.SFP_STATUS_BIT_INSERTED:
                return error_desc
        except Exception as e:
