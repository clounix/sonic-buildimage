...
static ssize_t cpld_debug_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    int ret;

    check_p(g_cpld_drv);
    check_p(g_cpld_drv->get_debug);

    ret = g_cpld_drv->get_debug(buf, PAGE_SIZE);
    if (ret < 0) {
        CPLD_ERR("get cpld debug failed, ret: %d\n", ret);
        return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SYSFS_DEV_ERROR);
    }
    return ret;
}

static ssize_t cpld_debug_store(struct switch_obj *obj, struct switch_attribute *attr,
                   const char* buf, size_t count)
{
    int ret;

    check_p(g_cpld_drv);
    check_p(g_cpld_drv->set_debug);
...
static ssize_t cpld_loglevel_show(struct switch_obj *obj, struct switch_attribute *attr, char *buf)
{
    int ret;

    check_p(g_cpld_drv);
    check_p(g_cpld_drv->get_loglevel);

    ret = g_cpld_drv->get_loglevel(buf, PAGE_SIZE);
    if (ret < 0) {
        CPLD_ERR("get cpld loglevel failed, ret: %d\n", ret);
        return (ssize_t)snprintf(buf, PAGE_SIZE, "%s\n", SYSFS_DEV_ERROR);
    }
    return ret;
}

static ssize_t cpld_loglevel_store(struct switch_obj *obj, struct switch_attribute *attr,
                   const char* buf, size_t count)
{
    int ret;

    check_p(g_cpld_drv);
    check_p(g_cpld_drv->set_loglevel);
...
static struct switch_attribute cpld_fw_version_attr = __ATTR(hw_version, S_IRUGO, cpld_fw_version_show, NULL);
