...
try:
    ...
    from sonic_platform.sfp_8472 import Sfp_8472
...
class Sfp(SfpBase):
    ...
    def __init__(self, index, sfp_conf):
        self.__index = index
        self.__conf = sfp_conf
        self.__api_helper = APIHelper()

        self.__attr_path_prefix = '/sys/s3ip/transceiver/eth{}/'.format(
            self.__index+1)

        self.__presence_attr = None
        self.__eeprom_path = None

        self.__eeprom_path = TRANSCEIVER_PATH + \
            'eth{}/'.format(self.__index+1)+'eeprom'
        self.__presence_attr = TRANSCEIVER_PATH + \
            'eth{}/'.format(self.__index+1)+'present'
        self.__reset_attr = TRANSCEIVER_PATH + \
            'eth{}/'.format(self.__index+1)+'reset'
        self.__lpmode_attr = TRANSCEIVER_PATH + \
            'eth{}/'.format(self.__index+1)+'low_power_mode'
        ...
        self.qsfp_8436 = QSfp_SFF8436(index)
        self.qsfp_cmis = QSfp_CMIS(index)
        self.sfp_8472 = Sfp_8472(index)
        self.is_cmis = False
    ...
    def _detect_sfp_type(self, sfp_type):
        ...
        if eeprom_raw:
            if eeprom_raw[0] in SFP_TYPE_CODE_LIST:
                ...
                # xcvrd.py  unable to set SI settings when sfp_type is QSFP
    ...
    def get_presence(self):
        ...
        try:
            attr_rv = self.__api_helper.read_one_line_file(
                self.__attr_path_prefix + 'present')
            if attr_rv:
                val = int(attr_rv.strip(), 16)
                return (val & 0x1) == 1
        except Exception:
            pass
        return False
...
##############################################
    ...
    def get_transceiver_info(self):
        ...
        if self.sfp_type == SFP_TYPE:
            info = self.sfp_8472.get_transceiver_info()
        elif self.is_cmis:
            info = self.qsfp_cmis.get_transceiver_info()
        else:
            info = self.qsfp_8436.get_transceiver_info()
        ...
        if info.get('vendor_rev', 'N/A') == 'N/A':
            info['vendor_rev'] = info.get('hardware_rev', 'N/A')
        if info.get('hardware_rev', 'N/A') == 'N/A':
            info['hardware_rev'] = info.get('vendor_rev', 'N/A')

        mandatory_defaults = {
            'type': 'N/A', 'type_abbrv_name': 'N/A', 'connector': 'N/A',
            'encoding': 'N/A', 'ext_identifier': 'N/A', 'ext_rateselect_compliance': 'N/A',
            'cable_type': 'N/A', 'cable_length': 'N/A', 'nominal_bit_rate': 'N/A',
            'specification_compliance': 'N/A', 'vendor_date': 'N/A', 'vendor_oui': 'N/A',
            'manufacturer': 'N/A', 'model': 'N/A', 'hardware_rev': 'N/A', 'vendor_rev': 'N/A',
            'serial': 'N/A', 'application_advertisement': 'N/A'
        }
    ...
    def get_transceiver_bulk_status(self):
        ...
        """
        if self.sfp_type == SFP_TYPE:
            return self.sfp_8472.get_transceiver_bulk_status()
    ...
    def get_transceiver_threshold_info(self):
        ...
        """
        if self.sfp_type == SFP_TYPE:
            return self.sfp_8472.get_transceiver_threshold_info()
        if self.is_cmis == True:
    ...
    def get_reset_status(self):
        ...
        """
        if self.sfp_type == SFP_TYPE:
            return self.sfp_8472.get_reset_status()
        if self.is_cmis == True:
            return self.qsfp_cmis.get_reset_status()
        else:
            return self.qsfp_8436.get_reset_status()
    ...
    def get_rx_los(self):
        ...
        """
        if self.sfp_type == SFP_TYPE:
            return self.sfp_8472.get_rx_los()
        if self.is_cmis == True:
    ...
    def get_tx_fault(self):
        ...
        """
        if self.sfp_type == SFP_TYPE:
            return self.sfp_8472.get_tx_fault()
        if self.is_cmis == True:
    ...
    def get_tx_disable(self):
        ...
        """
        if self.sfp_type == SFP_TYPE:
            return self.sfp_8472.get_tx_disable()
        if self.is_cmis == True:
    ...
    def get_tx_disable_channel(self):
        ...
        """
        if self.sfp_type == SFP_TYPE:
            return self.sfp_8472.get_tx_disable_channel()
        if self.is_cmis == True:
    ...
    def get_lpmode(self):
        ...
        """
        if self.sfp_type == SFP_TYPE:
            return self.sfp_8472.get_lpmode()
        if self.is_cmis == True:
    ...
    def get_temperature(self):
        ...
        """
        if self.sfp_type == SFP_TYPE:
            return self.sfp_8472.get_temperature()
        if self.is_cmis == True:
    ...
    def get_voltage(self):
        ...
        """
        if self.sfp_type == SFP_TYPE:
            return self.sfp_8472.get_voltage()
        if self.is_cmis == True:
    ...
    def get_tx_bias(self):
        ...
        """
        if self.sfp_type == SFP_TYPE:
            return self.sfp_8472.get_tx_bias()
        if self.is_cmis == True:
    ...
    def get_rx_power(self):
        ...
        """
        if self.sfp_type == SFP_TYPE:
            return self.sfp_8472.get_rx_power()
        if self.is_cmis == True:
    ...
    def get_tx_power(self):
        ...
        """
        if self.sfp_type == SFP_TYPE:
            return self.sfp_8472.get_tx_power()
        if self.is_cmis == True:
    ...
    def tx_disable(self, tx_disable):
        ...
        """
        if self.sfp_type == SFP_TYPE:
            return self.sfp_8472.tx_disable()
        if self.is_cmis == True:
    ...
    def tx_disable_channel(self, channel, disable):
        ...
        """
        if self.sfp_type == SFP_TYPE:
            return self.sfp_8472.tx_disable_channel()
        if self.is_cmis == True:
    ...
    def set_lpmode(self, lpmode):
        ...
        """
        if self.sfp_type == SFP_TYPE:
            return self.sfp_8472.set_lpmode()
        if self.is_cmis == True:
