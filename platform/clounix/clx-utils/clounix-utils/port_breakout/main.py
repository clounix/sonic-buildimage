...
import port_breakout.libpcfgparsing as PCFG
...
DEFAULT_MEDIA_PREFIX = 'cr'  # for copper
...
PORT_CONFIG_CLX = 'lightning-clx8000-48c8d.dsh'
...
class PortBreakout:
    ...
    def show(self, fpport_index_list=None, show_index_0based=True, print_msg=True):
        if fpport_index_list and len(list(fpport_index_list)) != 0:
    ...
    def _do_breakout(self, fpport_index, breakout_type):
        ...
        fp_max_speed = PCFG.MAX_SPEED_PER_LANE*sum(len(self.cur_port_profile.get_user_port_lanes(usr_port, fpport_index)) for usr_port in fp_usr_ports)
        ...
        for port_cnt in range(1, PORT_BREAKOUT_TYPE_2_NUM_MAP[breakout_type] + 1):
            ...
            p_lanes = [fp_lanes_reverse.pop() for i in range(int(p_lane_count))]
            p_alias = p_alias_prefix + '/' + str(port_cnt)
            p_speed = fp_max_speed / PORT_BREAKOUT_TYPE_2_NUM_MAP[breakout_type]

            #fix the bug:100G/2lanes unable to an-lt 
            if breakout_type == PORT_BREAKOUT_TYPE_2X and fp_max_speed == PCFG.MAX_SPEED_PER_LANE*PCFG.LANE_NUM_PER_MACRO :
                p_speed = fp_max_speed / PORT_BREAKOUT_TYPE_2_NUM_MAP[PORT_BREAKOUT_TYPE_4X]
    ...
    def reset_config_db_json(self):
        ...
        boot_image = re.search(r"Current: SONiC-OS-([^\s]*)", res.decode('utf-8')).group(1)
    ...
    def _generate_port_config_ini(self):
        header = ('# name', 'lanes', 'alias', 'speed', 'index', 'mtu', 'fec')
        ...
        for fpport_index in orig_fpports:
            ...
            for usr_p in usr_ports:
                usr_port_lanes = self.cur_port_profile.get_user_port_lanes(usr_p, fpport_index)
                usr_port_speed = self.cur_port_profile.get_user_port_speed(usr_p, fpport_index)
                if 100000 <= int(usr_port_speed):
                    fec = 'rs'
                else:
                    fec = ''
                item = [
                    ...
                    ','.join(usr_port_lanes),
                    alias_tpl.format(int(next_name_index)),
                    usr_port_speed,
                    fpport_index,
                    '9126',
                    fec
    ...
    def _generate_port_config_clx(self):
        ...
        port_admin_list = list()
        port_speed_list = list()
        port_an_lt_list = list()
        port_fec_list = list()
        pre_attr_list = list()
        ...
        for fpport_index in self.cur_port_profile.get_fpports():
            for usr_p in self.cur_port_profile.get_fpport_user_ports(fpport_index):
                ...
                port = self.cur_port_profile.get_user_port_clx_port(usr_p, fpport_index)
                user_port_2_clx_port[usr_p] = (unit, port)
                next_unit_port_index[unit] += 1


        # port-map
        for logic_lane in self.cur_port_profile.get_user_ports():
            p_unit, p_port = self.cur_port_profile.port_clx[PCFG.KEY_LOGIC_LANE_2_CLX_PORT][logic_lane]
            p_max_speed = self.cur_port_profile.port_clx[PCFG.KEY_CLX_PORT_INFO_V_MAX_SPEED][logic_lane]
            p_macro, p_lane = logic_lane_2_macro_lane(logic_lane)
            p_active = self.cur_port_profile.port_clx[PCFG.KEY_CLX_PORT_INFO_V_ACTIVE][logic_lane]
        ...
        port_map_list.sort(key=lambda x: int(x['clx_port']))
        ...
        for fpport_index in self.cur_port_profile.get_fpports():
            ...
            for usr_p in user_ports:
                ...
                # update port active
                port_map_list[int(p_port)]['active'] = 'true'

                # speed
                p_speed = int(self.cur_port_profile.get_user_port_speed(usr_p, fpport_index)) // 1000
                ...
                # admin
                try:
                    p_admin = self.cur_port_profile.port_clx[PCFG.KEY_ADMIN][(p_unit ,p_port)]
                except Exception as err:
                    p_admin = 'enable'

                port_admin_list.append(
                    {'unit': p_unit,
                     'clx_port': p_port,
                     'admin': p_admin
                     }
                ...
                # an-lt
                try:
                    p_an_lt = self.cur_port_profile.port_clx[PCFG.KEY_AN][(p_unit ,p_port)]
                except Exception as err:
                    p_an_lt = 'lt'
                port_an_lt_list.append(
                    {'unit': p_unit,
                     'clx_port': p_port,
                     'an_lt': p_an_lt
                    }
                ...
                pre_attr_list.append(
                {"unit": p_unit,
                 "clx_port": p_port,
                 'speed': p_speed,
                 'medium': p_medium,
                 "lane_cnt": len(p_lanes)}
                )
        ...
        self.new_port_config_clx = render(
            ...
            port_admin = port_admin_list,
            port_mdio = port_mdio_list,
            port_fec = port_fec_list,
            port_an_lt = port_an_lt_list,
            port_attr = pre_attr_list,
            **cpi_data
        )
    ...
    def _generate_cpi_port_config_clx(self):
        data_dict = {
            ...
            'cpi_speed':  self.cur_port_profile.port_clx[PCFG.KEY_SPEED].get(PCFG.KEY_CPI_PORT_INFO, []),
            'cpi_medium_type':  self.cur_port_profile.port_clx[PCFG.KEY_MEDIUM_TYPE].get(PCFG.KEY_CPI_PORT_INFO, []),
            'cpi_fec':  self.cur_port_profile.port_clx[PCFG.KEY_FEC].get(PCFG.KEY_CPI_PORT_INFO, []),
    ...
    def recover_from_json(self, json_file=PORT_BREAKOUT_JSON_FILE):
        ...
        for fp, fv in fpports_cfg.items():
        ...
        for b_type, p_list in breakout_fp_dict.items():
...
class PortProfile:
    ...
    def get_user_ports(self):
        return sorted(self.port_clx[PCFG.KEY_CLX_PORT_INFO_V_INDEX].keys(), key=int)
    ...
    def get_fpport_pre_emphasis(self, fpport_index):
        ...
        """
        DEFAULT_PRE_EMPHASIS_OPT_CFG_FILE_NAME = 'lightning-clx8000-48c8d.dsh'
    ...
    def get_fpport_user_ports(self, fpport_index):
        fp = self.phy_port_info[fpport_index]
        usr_p_infos = list(fp[PCFG.KEY_PHY_PORT_INFO_V_USER_PORT].items())
    ...
    def update_fpport_user_port_info(self, fpport_index, usr_port_infos):
        ...
        tmp_fp_lane_list = []
        for usr_p, v in usr_port_infos.items():
