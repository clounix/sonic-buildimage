...
static int via_cputemp_probe(struct platform_device *pdev)
{
    ...
    if (err)
        return err;
    /* clounix add */
    s3ip_cpu_temp_sensor_add(&pdev->dev, via_cputemp_attributes, 0);
    /* clounix end */
    ...
    data->hwmon_dev = hwmon_device_register_with_groups(&pdev->dev, DRVNAME, NULL, NULL);
...
static int via_cputemp_remove(struct platform_device *pdev)
{
    ...
    s3ip_cpu_temp_sensor_del(&pdev->dev, via_cputemp_attributes);
...
int __init via_cputemp_init(void)
{
    int err;
